...
export type CampaignInput = { schoolId: Id<"schools">; programmeSlug: string; programmeName: string; programmeDescription?: string; intakeSlug: string; intakeName: string; cycleLabel: string; targetClassId?: Id<"classes">; opensAt: number; closesAt: number; startsAt?: number; schemaVersion: string; fields: CampaignFieldInput[]; requirements: CampaignRequirementInput[]; declarationTitle: string; declarationBody: string; declarationPurpose: string; productSlug: string; productName: string; amountMinor: number; currency: string; refundPolicyKey: string; feeDisclosure: string; priceApprovalEvidenceId?: Id<"schoolApprovalEvidence">; effectiveFrom: number; effectiveTo?: number };
export type CampaignBundle = CampaignIds & Omit<CampaignInput, "schoolId" | "programmeDescription" | "targetClassId" | "startsAt" | "effectiveTo" | "priceApprovalEvidenceId"> & { lifecycle: "draft" | "published"; draftRevision: string; applicationLink: ApplicationLinkV1; programmeDescription: string | null; startsAt: number | null; effectiveTo: number | null; formVersion: number; priceApprovalEvidenceId: Id<"schoolApprovalEvidence"> | null; priceApprovalSubjectKey: string };
...
export type DocumentMetadata = { documentKey: string; requirementId: Id<"admissionsDocumentRequirements"> | null; category: string; fileName: string; mimeType: string; byteSize: number; version: number; submittedState: string; currentState: string; sensitivity: string };
...
export type PurchaseTerms = { amountMinor: number; currency: string; feeDisclosure: string; refundPolicyKey: string; termsDigest: string };
export type PurchaseAttemptResult = { attemptId: Id<"admissionsPurchaseAttempts">; reference: string; state: string; amountMinor: number; currency: string; terms: PurchaseTerms; entitlementId: Id<"admissionsEntitlements"> | null; replayed: boolean };
export type WorkspaceResult = { schoolId: Id<"schools">; entitlements: Array<{ entitlementId: Id<"admissionsEntitlements">; state: string; applicationId: Id<"admissionsApplications"> | null; createdAt: number }>; applications: Array<{ applicationId: Id<"admissionsApplications">; publicId: string; state: string; draftVersion: number; currentRevision: number; updatedAt: number }>; attempts: Array<Omit<PurchaseAttemptResult, "attemptId" | "replayed"> & { createdAt: number }> };
...
export const markAttemptRecoveryOutcomeRef = makeFunctionReference<
  "mutation",
  { attemptId: Id<"admissionsPurchaseAttempts">; outcome: "failed" | "manual_attention" | "verification_pending"; failureCode: string },
  null
>("functions/admissions/payments:markAttemptRecoveryOutcome");
...
export const approveCampaignPriceTermsRef = makeFunctionReference<"mutation", { schoolId: Id<"schools">; priceId: Id<"admissionsProductPrices">; expectedSubjectKey: string }, { approvalEvidenceId: Id<"schoolApprovalEvidence">; subjectKey: string; replayed: boolean }>("functions/admissions/catalogue:approveCampaignPriceTerms");
export const approveCampaignPublicationRequirementsRef = makeFunctionReference<"mutation", CampaignIds & { draftRevision: string }, { approvedCount: number; replayedCount: number }>("functions/admissions/catalogue:approveCampaignPublicationRequirements");
export const publishCampaignRef = makeFunctionReference<"mutation", CampaignIds & { draftRevision: string }, null>("functions/admissions/catalogue:publishCampaign");
...
export type DecisionWorkflow = { decision: null | { decisionId: Id<"admissionsDecisions">; snapshotId: Id<"admissionsSubmissionSnapshots"> | null; version: number; state: "in_evaluation" | "ready_for_decision" | "waitlisted" | "accepted" | "rejected"; reasonCode: string | null; guardianMessage: string | null; decidedAt: number }; evaluations: Array<{ evaluationId: Id<"admissionsEvaluations">; type: "entrance_assessment" | "interview"; state: "scheduled" | "completed" | "cancelled"; scheduledAt: number | null; completedAt: number | null; resultCode: string | null; score: number | null; version: number }>; readiness: { ready: boolean; acceptanceReady: boolean; blockers: string[] } };
export const getDecisionWorkflowRef = makeFunctionReference<"query", { schoolId: Id<"schools">; applicationId: Id<"admissionsApplications">; evaluationLimit?: number }, DecisionWorkflow>("functions/admissions/staff:getLatestReviewState");
export const getLatestReviewStateRef = getDecisionWorkflowRef;
export const recordEvaluationRef = makeFunctionReference<"mutation", { schoolId: Id<"schools">; applicationId: Id<"admissionsApplications">; type: "entrance_assessment" | "interview"; state: "scheduled" | "completed" | "cancelled"; scheduledAt?: number; resultCode?: string; score?: number }, { evaluationId: Id<"admissionsEvaluations">; version: number }>("functions/admissions/staff:recordEvaluation");
export const markReadyForDecisionRef = makeFunctionReference<"mutation", { schoolId: Id<"schools">; applicationId: Id<"admissionsApplications"> }, { decisionId: Id<"admissionsDecisions">; version: number }>("functions/admissions/staff:markReadyForDecision");
export const resumeWaitlistedRef = makeFunctionReference<"mutation", { schoolId: Id<"schools">; applicationId: Id<"admissionsApplications">; reasonCode: string }, { decisionId: Id<"admissionsDecisions">; version: number }>("functions/admissions/staff:resumeWaitlisted");
export const reopenDecisionRef = makeFunctionReference<"mutation", { schoolId: Id<"schools">; applicationId: Id<"admissionsApplications">; reasonCode: string }, { decisionId: Id<"admissionsDecisions">; version: number }>("functions/admissions/staff:reopenDecision");
...
export const getDocumentAccessRef = makeFunctionReference<"mutation", { schoolId: Id<"schools">; documentKey: string; action: "view" | "download"; reason: string }, { status: "unavailable" } | { status: "available"; url: string; expiresAt: number }>("functions/admissions/staff:getDocumentAccess");
export const consumeStaffDocumentAccessGrantRef = makeFunctionReference<"mutation", { token: string }, { status: "unavailable" } | { status: "available"; upstreamUrl: string; fileName: string; contentType: string; byteSize: number; action: "view" | "download" }>("functions/admissions/staff:consumeDocumentAccessGrant");
export const recordDecisionRef = makeFunctionReference<"mutation", { schoolId: Id<"schools">; applicationId: Id<"admissionsApplications">; state: "waitlisted" | "accepted" | "rejected"; reasonCode: string; guardianMessage: string }, { decisionId: Id<"admissionsDecisions">; version: number; replayed: boolean }>("functions/admissions/staff:recordDecision");
...
export const createAttemptRef = makeFunctionReference<"mutation", { schoolSlug: string; productSlug: string; idempotencyKey: string }, PurchaseAttemptResult>("functions/admissions/payments:createAttempt");
export const initializeAttemptRef = makeFunctionReference<"action", { reference: string; confirmedTermsDigest: string; returnOrigin?: string }, { reference: string; state: string; authorizationUrl: string; replayed: boolean }>("functions/admissions/payments:initializeAttempt");
export const verifyReturnRef = makeFunctionReference<"action", { reference: string }, PurchaseAttemptResult>("functions/admissions/payments:verifyReturn");
...
export const getOwnDocumentAccessRef = makeFunctionReference<"mutation", { documentKey: string; action: "view" | "download" }, { status: "unavailable" } | { status: "available"; url: string; expiresAt: number }>("functions/admissions/documents:getOwnAccess");
export const consumeGuardianDocumentAccessGrantRef = makeFunctionReference<"mutation", { token: string }, { status: "unavailable" } | { status: "available"; upstreamUrl: string; fileName: string; contentType: string; byteSize: number; action: "view" | "download" }>("functions/admissions/documents:consumeOwnAccessGrant");
