...
export const submit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (application.state === "changes_requested" && correctionSnapshotId) {
      ...
      const priorItems = new Map(snapshotItems.map((item) => [item.itemKey, item]));
      const correctionLabel = (fieldKey: string) => fieldKey === "profile" ? "Applicant profile" : fieldKey === "primaryContact" ? "Primary contact" : fieldKey === "requestedEntryLabel" ? "Requested entry" : fields.find((field) => field.fieldKey === fieldKey)?.label ?? "Requested answer";
      const unchangedFields = scope.fieldKeys.filter((fieldKey) => {
        const itemKey = fieldKey === "profile" || fieldKey === "primaryContact" || fieldKey === "requestedEntryLabel" ? fieldKey : `answer:${fieldKey}`;
        const prior = priorItems.get(itemKey);
        const currentAnswer = answersByKey.get(fieldKey);
        const currentValue = fieldKey === "profile" ? JSON.stringify(canonicalProfile) : fieldKey === "primaryContact" ? JSON.stringify(contact) : fieldKey === "requestedEntryLabel" ? application.requestedEntryLabel?.trim() : currentAnswer?.serializedValue;
        const currentValueType = currentAnswer?.valueType;
        return prior?.serializedValue === currentValue && (fieldKey === "profile" || fieldKey === "primaryContact" || fieldKey === "requestedEntryLabel" || prior?.valueType === currentValueType);
      });
      if (unchangedFields.length) admissionsError("APPLICATION_INCOMPLETE", `Update requested items before resubmitting: ${unchangedFields.map(correctionLabel).join(", ")}`);
    }
    ...
    const canonical = { profile: canonicalProfile, primaryContact: contact, requestedEntryLabel: application.requestedEntryLabel.trim(), answers: answers.map((answer) => ({ fieldKey: answer.fieldKey, valueType: answer.valueType, serializedValue: answer.serializedValue, dataClass: answer.dataClass, valueVersion: answer.valueVersion })).sort((a, b) => a.fieldKey.localeCompare(b.fieldKey)), documents: documentManifest, declaration: { id: String(declaration._id), digest: declaration.bodyDigest } };
