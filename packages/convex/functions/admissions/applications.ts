...
export const submit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!profile || !boundDefinitionsRemainValid || !intake || intake.status !== "open" || now < intake.opensAt || now > intake.closesAt) admissionsError("APPLICATION_INCOMPLETE", "Applicant profile, valid bound definitions, and an open intake are required");
