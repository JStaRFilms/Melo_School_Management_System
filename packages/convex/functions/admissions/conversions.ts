...
export const executeAcceptedConversion = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    let photo: any = null;
    if (args.photoDocumentId) { photo = await ctx.db.get(args.photoDocumentId); if (!photo || photo.schoolId !== application.schoolId || photo.applicationId !== application._id || photo.state !== "accepted" || photo.category !== "photo") throw new ConvexError("CONVERSION_RESOLUTION_REQUIRED"); }
