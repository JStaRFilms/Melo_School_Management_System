...
export const getDocumentAccess = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const sensitiveDownload = args.action === "download" && (document.sensitivity === "highly_sensitive" || document.sensitivity === "financial_security");
    const freshEnough = !sensitiveDownload || await hasFreshAuthentication(ctx);
