...
export const bindUpload = mutation({
  ...
  handler: async (ctx, args): Promise<{ documentKey: string; state: "uploaded"; version: number }> => {
    const { guardian, application } = await requireOwnedApplication(ctx, args.applicationId);
    assertEditable(application.state);
    const [requirement, bound] = await Promise.all([
      ctx.db.get("admissionsDocumentRequirements", args.requirementId),
    ...
    const now = Date.now();
    const documentKey = opaqueKey("doc_");
    const documentId: Id<"admissionsDocuments"> = await ctx.db.insert("admissionsDocuments", { schoolId: application.schoolId, applicationId: application._id, requirementId: requirement._id, category: requirement.category, documentKey, storageId: args.storageId, fileName: args.fileName.trim().slice(0, 256) || "upload", mimeType: contentType, byteSize: metadata.size, sha256: metadata.sha256, version, state: "uploaded", sensitivity: requirement.sensitivity, uploadedByGuardianId: guardian._id, ...(prior.length ? { supersedesDocumentId: prior.sort((left, right) => right.version - left.version)[0]._id } : {}), retentionHold: false, createdAt: now, updatedAt: now });
    ...
    return { documentKey, state: "uploaded", version };
  },
});
