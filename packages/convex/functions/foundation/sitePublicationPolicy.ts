...
const fieldRules: ReadonlyArray<{ pattern: RegExp; kinds: readonly string[]; approvalClass: "standard" | "sensitive_public" | "identity" }> = [
  ...
  { pattern: /^about\.(values|story)\.ids$/, kinds: ["string_list"], approvalClass: "standard" },
  { pattern: /^about\.(values|story)\.[a-z0-9]+(?:-[a-z0-9]+)*\.(title|body)$/, kinds: ["text", "rich_text"], approvalClass: "standard" },
  { pattern: /^programmes\.ids$/, kinds: ["string_list"], approvalClass: "sensitive_public" },
  ...
  { pattern: /^admissions\.steps$/, kinds: ["string_list"], approvalClass: "sensitive_public" },
  { pattern: /^schoolLife\.(gallery|features\.ids)$/, kinds: ["string_list"], approvalClass: "standard" },
  ...
  { pattern: /^policies\.ids$/, kinds: ["string_list"], approvalClass: "sensitive_public" },
...
export type SiteField = { fieldId: string; value: { kind: string; value?: unknown; assetId?: unknown } };
...
export function assertRendererContent(rendererKey: string, schemaVersion: string, content: { fields: readonly SiteField[]; routeSeo: readonly RouteSeo[] }) {
  ...
  for (const field of content.fields) {
    ...
    if (field.value.kind === "string_list") {
      ...
      if (!Array.isArray(values) || values.length > 20 || values.some((item) => typeof item !== "string" || !item.trim() || item.length > 360 || /[<>]/.test(item))) throw new ConvexError("Invalid published list value");
    }
  }
...
export function expectedAssetUse(fieldId: string): { kinds: readonly string[]; purposes: readonly string[]; channel: "site" } | null {
