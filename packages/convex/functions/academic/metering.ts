  ...
  handler: async (ctx, args) => {
    ...
    const results = await Promise.all(allocations.map(async (alloc) => {
      ...
      const availableUnits = Math.max(
        0,
        allocatedUnits - alloc.consumedUnits - alloc.reservedUnits
      );
      const rawUtilizationPercent =
        allocatedUnits > 0 ? (activeUsed / allocatedUnits) * 100 : 100;
      ...
      return {
        ...
        thresholdAlert,
        isHardStopped,
        isCritical90: !isHardStopped && rawUtilizationPercent >= critical,
        isWarning75:
          rawUtilizationPercent >= warning && rawUtilizationPercent < critical,
