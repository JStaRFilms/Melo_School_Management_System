...
async function fixture() {
  ...
  const ids = await t.run(async (ctx) => {
    ...
    await ctx.db.insert("academicTerms", {
      ...
      sessionId,
      name: "Current term",
      startDate: today - 10 * day + day / 2,
      endDate: today + 29 * day + day / 2,
      isActive: true,
      createdAt: 1,
      updatedAt: 1,
    });
    await ctx.db.insert("academicTerms", {
      ...
      sessionId,
      name: "Next term",
      startDate: today + 30 * day + day / 2,
      endDate: today + 59 * day + day / 2,
      isActive: false,
      createdAt: 1,
      updatedAt: 1,
    });
...
it("rejects no-proration annual periods shifted away from the contract cadence", async () => {
  const f = await fixture();
  const contractStart = today - 30 * day;
  const anniversary = (start: number) => {
    const date = new Date(start);
    const year = date.getUTCFullYear() + 1;
    const month = date.getUTCMonth();
    const calendarDay = Math.min(date.getUTCDate(), new Date(Date.UTC(year, month + 1, 0)).getUTCDate());
    return Date.UTC(year, month, calendarDay);
  };
  const contractId = await f.platform.mutation(commercial.createContract, {
    ...f.contractArgs,
    schoolId: f.otherSchoolId,
    effectiveFrom: contractStart,
    effectiveTo: anniversary(anniversary(contractStart)),
    overrideRate: { ...rate, cadence: "annually", proration: "none" },
    overrideReason: "Reviewed annual no-proration contract",
    setupHandling: "waived",
  });
  const shiftedStart = contractStart + day;
  await expect(f.platform.mutation(commercial.issueSubscriptionInvoice, {
    ...f.invoiceArgs,
    schoolId: f.otherSchoolId,
    contractId,
    periodStart: shiftedStart,
    periodEnd: anniversary(shiftedStart),
  })).rejects.toThrow("anchored to the contract start");
});
