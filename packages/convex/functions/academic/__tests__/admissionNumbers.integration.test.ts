...
it("creates atomically, replays the same intent, preserves manual identifiers and requires explicit counter decisions", async () => {
  const { t, schoolId, sessionId, policyId, classId } = await fixture();
  ...
  const input = {
    ...
    numberingSessionId: sessionId,
    numberingResetPeriod: "continuous",
  };
  ...
  )).rejects.toThrow("request intent changed");
...
it("rejects an automatic allocation when the active session changes after preview", async () => {
  ...
  const viewer = t.withIdentity({ subject: "owner", issuer: "test", tokenIdentifier: "test|owner" });
  const reviewed = await viewer.query(api.functions.academic.admissionNumbers.getAdmissionNumberPolicy, { schoolId, level: "Primary" });
  await t.run(async (ctx) => {
    await ctx.db.patch(sessionId, { isActive: false, updatedAt: 2 });
  ...
  await expect(viewer.mutation(api.functions.academic.studentEnrollment.createStudent, {
    firstName: "Stale",
    lastName: "Review",
    gender: "male",
    classId,
    admissionNumber: "",
    numberingVersion: reviewed.version,
    numberingFormatVersion: reviewed.formatVersion ?? undefined,
    numberingCounterKey: reviewed.counter?.key,
    numberingCounterVersion: reviewed.counter?.configVersion,
    numberingSessionId: reviewed.activeSessionId ?? undefined,
    numberingResetPeriod: reviewed.resetPeriod ?? undefined,
  })).rejects.toThrow(/changed; review again/);
});

it("corrects an existing student only with reason and explicit version-pinned advancement", async () => {
  const { t, schoolId, sessionId, classId, policyId } = await fixture();
  const viewer = t.withIdentity({
    subject: "owner",
    issuer: "test",
    tokenIdentifier: "test|owner",
