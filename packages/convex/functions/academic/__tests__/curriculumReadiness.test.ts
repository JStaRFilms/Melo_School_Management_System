...
describe("curriculum readiness aggregation", () => {
  ...
  it("derives one school-scoped readiness map and rejects non-admins", async () => {
    ...
    expect(result).toMatchObject({ counts: { topicCount: 1, sourceApprovedCount: 1, lessonPlanPreparedCount: 1, studentNotePreparedCount: 1, assignmentPreparedCount: 1, assessmentDraftedCount: 1, studentResourcePublishedCount: 1 }, rows: [{ title: "Fractions", studentPublicationStatus: "student_resource_published" }] });
