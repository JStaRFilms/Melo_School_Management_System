...
const fn = {
  ...
  platformWorkspace: makeFunctionReference<"query">("functions/academic/usageEntitlements:getPlatformEntitlementWorkspace"),
};
...
it("records top-ups separately and exception requests grant nothing until one append-only Platform decision", async () => {
  ...
  const requestArgs = { schoolId: f.schoolId, cycleId: f.cycleId, requestKey: "exception-request-1", meterType: "ocr_pages" as const, units: 7, reason: "Temporary reviewed school need", confirmation: "REQUEST" };
...
it("creates a bounded group pool and idempotently allocates matching-cycle branch units for the proprietor", async () => {
  const f = await setup();
  const poolArgs = { journalSchoolId: f.schoolId, groupId: f.groupId, entitlementVersionId: f.versionId, meterType: "ai_tokens" as const, idempotencyKey: "group-pool-creation-1", totalUnits: 50, startAt: today - day, endAt: today + 20 * day, reason: "Reviewed shared group pool", confirmation: "CONFIRM" };
  const poolId = await f.platform.mutation(fn.pool, poolArgs) as Id<"usageGroupPools">;
  expect(await f.platform.mutation(fn.pool, poolArgs)).toBe(poolId);
  await expect(f.platform.mutation(fn.pool, { ...poolArgs, totalUnits: 51 })).rejects.toThrow("bound to different pool terms");
  const unrelatedSchoolId = await f.t.run(ctx => ctx.db.insert("schools", { name: "Unrelated", slug: `unrelated-${Math.random()}`, status: "active", createdAt: 1, updatedAt: 1 }));
  await expect(f.platform.mutation(fn.pool, {
    ...poolArgs,
    journalSchoolId: unrelatedSchoolId,
    idempotencyKey: "unrelated-pool-creation",
  })).rejects.toThrow("branch of the target group");
  const poolState = await f.t.run(async ctx => ({
    pools: await ctx.db.query("usageGroupPools").collect(),
    audits: (await ctx.db.query("auditEvents").collect()).filter(event => event.action === "usage.group_pool_created"),
  }));
  expect(poolState.pools).toHaveLength(1);
  expect(poolState.audits).toHaveLength(1);
  expect(poolState.audits[0].groupId).toBe(f.groupId);
...
it("closes with provenance-validated effective-at-end totals while preserving cumulative expired source history", async () => {
  ...
  const requestId = await f.owner.mutation(fn.request, {
    schoolId: f.schoolId,
    cycleId: f.cycleId,
    requestKey: "expiring-exception-request",
    meterType: "ocr_pages",
    units: 7,
  ...
  const poolId = await f.platform.mutation(fn.pool, {
    ...
    meterType: "ai_tokens",
    idempotencyKey: "temporary-group-pool",
    totalUnits: 40,
    startAt: today - day,
    endAt: today + 20 * day,
...
it("closes an ended cycle with no active reservations, preserves its balances, and starts a clean next cycle", async () => {
  ...
  await f.t.run(async ctx => {
    const [aiMeter, storageMeter] = await Promise.all([
      ctx.db.query("usageMeterAllocations").withIndex("by_school_and_meter", q => q.eq("schoolId", f.schoolId).eq("meterType", "ai_tokens")).unique(),
      ctx.db.query("usageMeterAllocations").withIndex("by_school_and_meter", q => q.eq("schoolId", f.schoolId).eq("meterType", "storage_bytes")).unique(),
    ]);
    if (!aiMeter || !storageMeter) throw new Error("missing meter");
    await ctx.db.patch(aiMeter._id, { consumedUnits: 30, reservedUnits: 1 });
    await ctx.db.patch(storageMeter._id, { consumedUnits: 450, activeStorageBytes: 300, trashStorageBytes: 100, tempStorageBytes: 50 });
  });
  const closeArgs = {
    schoolId: f.schoolId,
    cycleId: f.cycleId,
    expectedMeters: [
      ...
      { meterType: "storage_bytes" as const, allocatedUnits: 1100, consumedUnits: 450 },
    ],
  ...
  try {
    ...
    const state = await f.t.run(async ctx => ({
      ...
      storage: await ctx.db.query("usageMeterAllocations").withIndex("by_school_and_meter", q => q.eq("schoolId", f.schoolId).eq("meterType", "storage_bytes")).unique(),
    }));
    ...
    expect(state.storage).toMatchObject({ cycleId: nextCycleId, consumedUnits: 450, activeStorageBytes: 300, trashStorageBytes: 100, tempStorageBytes: 50 });
  } finally {
    clock.mockRestore();
  }
});
