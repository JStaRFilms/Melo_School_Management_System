...
describe("Private registered draft lifecycle", () => {
  ...
  it("reserves teacher drafts before auth provisioning and rejects the wrong form", async () => {
    const h = await setup();
    const staffScope = { schoolId: h.schoolId, formKey: "staff_onboarding" };
    const staff = await h.user.mutation(drafts.beginFormDraft, { ...staffScope, schemaVersion: 1 });
    await h.user.mutation(internal.functions.academic.academicSetup.reserveTeacherProvisioningInternal, {
      schoolId: h.schoolId,
      email: "reserved@example.test",
      draftId: staff.draftId,
    ...
    await expect(h.user.mutation(drafts.saveFormDraft, {
      ...
      expectedRevision: 0,
      schemaVersion: 1,
      payload: { name: "Changed", email: "reserved@example.test" },
    })).rejects.toThrow(/submission in progress/);
    await h.user.mutation(internal.functions.academic.academicSetup.releaseTeacherProvisioningInternal, {
      schoolId: h.schoolId,
      email: "reserved@example.test",
      draftId: staff.draftId,
      expectedDraftRevision: 0,
    ...
    const wrong = await h.begin();
    await expect(h.user.mutation(internal.functions.academic.academicSetup.reserveTeacherProvisioningInternal, {
      schoolId: h.schoolId,
      email: "wrong-form@example.test",
      draftId: wrong.draftId,
      expectedDraftRevision: 0,
