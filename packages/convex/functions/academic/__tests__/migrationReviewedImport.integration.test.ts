...
describe("R1 reviewed import remediation", () => {
  ...
  it("rejects accidental collisions and permits only explicit same-tenant merge", async () => {
    ...
    await f.t.run((ctx) =>
      ctx.db.patch(existingStudentId, { enrollmentStatus: "transferred_out" }),
    );
    const inactiveOptions = await f.session.query(
      api.functions.academic.migrationWorkspace.getWorkspaceReviewOptions,
      { schoolId: f.schoolId, workspaceId },
    );
    expect(inactiveOptions.students.map((item) => item.id)).not.toContain(
      existingStudentId,
    );
    await expect(
      f.session.mutation(
    ...
    await f.t.run((ctx) =>
      ctx.db.patch(existingStudentId, { enrollmentStatus: "active" }),
    );
    await f.session.mutation(
      api.functions.academic.migrationAutosave.reviewStagedRecord,
      {
    ...
    await approveAll(f, workspaceId);
    await f.t.run((ctx) =>
      ctx.db.patch(existingStudentId, { enrollmentStatus: "withdrawn" }),
    );
    await expect(
      f.session.mutation(
        api.functions.academic.migrationMerge.commitImportWorkspace,
  ...
  it("rejects foreign placement/family, missing explicit mapping, stale review and changed numbering", async () => {
    ...
    await approveAll(f, workspaceId);
    await f.t.run((ctx) => ctx.db.patch(f.classId, { isArchived: true }));
    const reviewOptions = await f.session.query(
      api.functions.academic.migrationWorkspace.getWorkspaceReviewOptions,
      { schoolId: f.schoolId, workspaceId },
    );
    expect(reviewOptions.classes.map((item) => item.id)).not.toContain(f.classId);
    await expect(
      f.session.mutation(
        api.functions.academic.migrationMerge.commitImportWorkspace,
        { schoolId: f.schoolId, workspaceId },
    ...
    await f.t.run((ctx) => ctx.db.patch(f.classId, { isArchived: false }));
  ...
  it("finds active review options beyond archived lifecycle rows", async () => {
    const f = await fixture();
    const workspaceId = await createWorkspace(f);
    const now = Date.now();
    const lateOptions = await f.t.run(async (ctx) => {
      for (let index = 0; index < 205; index += 1) {
        await ctx.db.insert("classes", {
          schoolId: f.schoolId,
          name: `Archived class ${index}`,
          level: "Primary",
          isArchived: true,
          createdAt: now,
          updatedAt: now,
      ...
      for (let index = 0; index < 505; index += 1) {
        await ctx.db.insert("students", {
          ...
          classId,
          userId: f.studentUserIds[0],
          admissionNumber: `ARCHIVED-${index}`,
          isArchived: true,
          createdAt: now,
          updatedAt: now,
        });
      ...
      const studentId = await ctx.db.insert("students", {
        ...
        classId,
        userId: f.studentUserIds[1],
        admissionNumber: "LATE-ACTIVE",
        isArchived: false,
        enrollmentStatus: "active",
        createdAt: now,
        updatedAt: now,
      });
      return { classId, subjectId, studentId };
    });

    const options = await f.session.query(
      api.functions.academic.migrationWorkspace.getWorkspaceReviewOptions,
      { schoolId: f.schoolId, workspaceId },
    );
    expect(options.classes.map((item) => item.id)).toContain(lateOptions.classId);
    expect(options.subjects.map((item) => item.id)).toContain(lateOptions.subjectId);
    expect(options.students.map((item) => item.id)).toContain(lateOptions.studentId);
  });
  ...
  it("requires explicit grade mappings and applies reviewed raw40 policy snapshots", async () => {
    ...
    const archivedSubjectOptions = await f.session.query(
      api.functions.academic.migrationWorkspace.getWorkspaceReviewOptions,
      { schoolId: f.schoolId, workspaceId },
    );
    expect(
      archivedSubjectOptions.subjects.map((item) => item.id),
    ).not.toContain(f.subjectId);
    await expect(
      reviewGrade(f, record._id, { selectedStudentId: existingStudentId }),
    ).rejects.toThrow("subject is unavailable");
