...
describe("U1f typed group domain defaults", () => {
  ...
  it("real role, academic, report and calendar consumers use only explicitly adopted prospective defaults", async () => {
    ...
    const entrySheet = await f.owner.query(
    ...
    expect(entrySheet.settings.examInputMode).toBe("raw60_scaled_to_40");
    await expect(
      f.owner.mutation(
  ...
  it("rejects invalid or overlapping calendar templates before creating a version", async () => {
    ...
    await f.owner.mutation(endpoints.saveGroupDomainSetting, {
      ...
      setting: {
        ...
        value: {
          terms: [
            { name: "Third", startOffsetDays: 100, endOffsetDays: 150, resultCalculationMode: "standalone" },
            { name: "First", startOffsetDays: 0, endOffsetDays: 50, resultCalculationMode: "standalone" },
            { name: "Second", startOffsetDays: 60, endOffsetDays: 80, resultCalculationMode: "standalone" },
          ],
        },
      },
    });
    const saved = await f.owner.query(endpoints.getGroupDomainSetting, {
      groupId: f.groupId,
      domain: "calendar_template",
    });
    expect(saved).toMatchObject({
      defaults: { value: { terms: [{ name: "First" }, { name: "Second" }, { name: "Third" }] } },
    });
  });
});
