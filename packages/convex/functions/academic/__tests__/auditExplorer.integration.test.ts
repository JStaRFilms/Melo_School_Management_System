...
it("uses scoped group sources beyond unrelated global pages and preserves relinking history", async () => {
  ...
  const secondGroupId = await f.t.run(async ctx => {
    ...
    for (const suffix of ["a", "b"]) {
      await ctx.db.insert("auditEvents", {
        eventId: `same_timestamp_${suffix}`,
        ...
        module: "groups",
        action: `same_timestamp_${suffix}`,
        targetType: "school",
        targetId: String(f.schoolId),
        outcome: "success",
        safeSummary: "Same transaction pagination tie",
        correlationId: `same_timestamp_${suffix}`,
        retentionClass: "operational_7yr",
        createdAt: now + 50_000,
      });
    }
    await ctx.db.insert("auditEvents", {
      eventId: "newer_legacy_without_group_snapshot",
      timestamp: now + 100_000,
      actorKind: "system",
      actorEmailSnapshot: "system",
      schoolId: f.schoolId,
      module: "groups",
      action: "newer_legacy_current_branch",
      targetType: "school",
      targetId: String(f.schoolId),
      outcome: "success",
      safeSummary: "Newer legacy event without historical group evidence",
      correlationId: "newer_legacy_without_group_snapshot",
      retentionClass: "operational_7yr",
  ...
  expect(
    newGroupActions.filter((action) => !action.startsWith("same_timestamp_")),
  ).toEqual([
    "newer_legacy_current_branch",
    "new_group_snapshot",
    "legacy_current_branch",
  ]);
  expect(
    new Set(newGroupActions.filter((action) => action.startsWith("same_timestamp_"))),
  ).toEqual(new Set(["same_timestamp_a", "same_timestamp_b"]));
