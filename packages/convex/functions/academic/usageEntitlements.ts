...
async function audit(ctx: MutationCtx, schoolId: Id<"schools">, action: string, targetId: string, summary: string, actorPersonId?: Id<"persons">, groupId?: Id<"schoolGroups">) {
...
async function currentCycle(ctx: Context, schoolId: Id<"schools">, now = Date.now()) {
  ...
  return cycle && cycle.endAt > now ? cycle : null;
}
...
export const startUsageCycle = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const row of version.entitlement.allowances) {
      ...
      const carriedStorage = row.meterType === "storage_bytes" && existing[0]
        ? {
            consumedUnits: existing[0].consumedUnits,
            activeStorageBytes: existing[0].activeStorageBytes ?? 0,
            trashStorageBytes: existing[0].trashStorageBytes ?? 0,
...
export const requestUsageException = mutation({
  args: { schoolId: v.id("schools"), cycleId: v.id("usageCycles"), requestKey: v.string(), meterType: usageMeterType, units: v.number(), reason: v.string(), confirmation: v.string() },
  handler: async (ctx, args) => {
    ...
    const cycle = await ctx.db.get(args.cycleId); if (!cycle || cycle.schoolId !== args.schoolId || cycle.startAt > Date.now() || cycle.endAt <= Date.now() || !cycle.entitlement.allowances.some(row => row.meterType === args.meterType)) throw new ConvexError("Current cycle meter unavailable");
    const requestKey = bounded(args.requestKey, "Request key", 8);
    const reason = bounded(args.reason, "Exception reason", 8);
    const existing = await ctx.db.query("usageExceptionRequests").withIndex("by_school_and_request_key", q => q.eq("schoolId", args.schoolId).eq("requestKey", requestKey)).unique();
    if (existing) {
      if (existing.cycleId !== cycle._id || existing.meterType !== args.meterType || existing.units !== args.units || existing.reason !== reason || existing.requestedByPersonId !== auth.personId) throw new ConvexError("Conflicting exception request retry");
      return existing._id;
    }
    const id = await ctx.db.insert("usageExceptionRequests", { schoolId: args.schoolId, cycleId: cycle._id, requestKey, meterType: args.meterType, units: args.units, reason, requestedByPersonId: auth.personId, requestedAt: Date.now() });
...
export const decideUsageException = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existing = await ctx.db.query("usageExceptionDecisions").withIndex("by_request", q => q.eq("requestId", request._id)).unique(); if (existing) { if (existing.outcome !== args.outcome || existing.reason !== args.reason.trim() || existing.expiresAt !== args.expiresAt) throw new ConvexError("Conflicting exception decision retry"); return existing._id; }
    ...
    const id = await ctx.db.insert("usageExceptionDecisions", { requestId: request._id, outcome: args.outcome, reason: bounded(args.reason, "Decision reason", 8), expiresAt: args.expiresAt, grantId, createdAt: Date.now() });
...
export const createGroupPool = mutation({
  args: { journalSchoolId: v.id("schools"), groupId: v.id("schoolGroups"), entitlementVersionId: v.id("usageEntitlementVersions"), meterType: usageMeterType, idempotencyKey: v.string(), totalUnits: v.number(), startAt: v.number(), endAt: v.number(), reason: v.string(), confirmation: v.string() },
  handler: async (ctx, args) => {
    await platform(ctx, args.journalSchoolId, args.confirmation);
    validPeriod(args.startAt, args.endAt);
    safePositive(args.totalUnits, "Pool units");
    const key = bounded(args.idempotencyKey, "Pool creation ID", 8);
    const reason = bounded(args.reason, "Pool reason", 8);
    const journalLink = await ctx.db
      .query("schoolGroupBranches")
      .withIndex("by_group_and_school", q =>
        q.eq("groupId", args.groupId).eq("schoolId", args.journalSchoolId),
      )
      .unique();
    if (!journalLink)
      throw new ConvexError("Audit school must be a branch of the target group");
    const existing = await ctx.db
      .query("usageGroupPools")
      .withIndex("by_group_and_idempotencyKey", q =>
        q.eq("groupId", args.groupId).eq("idempotencyKey", key),
      )
      .unique();
    if (existing) {
      if (
        existing.journalSchoolId !== args.journalSchoolId ||
        existing.entitlementVersionId !== args.entitlementVersionId ||
        existing.meterType !== args.meterType ||
        existing.totalUnits !== args.totalUnits ||
        existing.startAt !== args.startAt ||
        existing.endAt !== args.endAt ||
        existing.reason !== reason
      ) throw new ConvexError("Pool creation ID is bound to different pool terms");
      return existing._id;
    }
    const version = await ctx.db.get(args.entitlementVersionId);
    const group = await ctx.db.get(args.groupId);
    if (!version || version.effectiveFrom > args.startAt || !group || group.status !== "active" || !version.entitlement.allowances.some(row => row.meterType === args.meterType)) throw new ConvexError("Effective group entitlement meter unavailable");
    const id = await ctx.db.insert("usageGroupPools", { groupId: group._id, journalSchoolId: args.journalSchoolId, entitlementVersionId: version._id, meterType: args.meterType, idempotencyKey: key, totalUnits: args.totalUnits, startAt: args.startAt, endAt: args.endAt, reason, createdAt: Date.now() });
    await audit(ctx, args.journalSchoolId, "usage.group_pool_created", id, "Created bounded group allowance pool; no branch allocation or purchase inferred", undefined, group._id);
    return id;
  },
});
...
export const getPlatformEntitlementWorkspace = query({
  args: { schoolId: v.id("schools") }, handler: async (ctx, args) => {
    if (!(await isGroupPlatformOperator(ctx))) throw new ConvexError("Forbidden: active Platform authority required");
    const versions = await ctx.db.query("usageEntitlementVersions").order("desc").take(100);
    const cycles = await ctx.db.query("usageCycles").withIndex("by_school", q => q.eq("schoolId", args.schoolId)).order("desc").take(100);
    const recentRequests = await ctx.db.query("usageExceptionRequests").withIndex("by_school", q => q.eq("schoolId", args.schoolId)).order("desc").take(100);
    const decisions = await Promise.all(recentRequests.map(request =>
      ctx.db.query("usageExceptionDecisions").withIndex("by_request", q => q.eq("requestId", request._id)).unique()));
    const requests = recentRequests.filter((_request, index) => !decisions[index]);
    const activeCycles = cycles.filter(cycle => cycle.status === "active");
    if (activeCycles.length > 1) throw new ConvexError("Overlapping usage cycles require reconciliation");
    const activeCycle = activeCycles[0];
    const endedCycle = activeCycle && activeCycle.endAt <= Date.now() ? activeCycle : null;
    const closingMeters = [];
    if (endedCycle) for (const allowance of endedCycle.entitlement.allowances) {
      const [effective, meter] = await Promise.all([
        effectiveAllowance(ctx, endedCycle, allowance.meterType, endedCycle.endAt),
        allocation(ctx, args.schoolId, allowance.meterType),
      ]);
      if (!effective || meter.cycleId !== endedCycle._id)
