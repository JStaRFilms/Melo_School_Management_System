...
function assertExpected(
  ...
  if (!context.policy || !context.session || !context.format || !context.counter || !context.period) {
    throw new ConvexError(
  ...
  if (
    ...
    (expected.expectedSessionId !== undefined &&
      expected.expectedSessionId !== context.session._id) ||
    (expected.expectedResetPeriod !== undefined &&
      expected.expectedResetPeriod !== context.period)
  ) {
    throw new ConvexError(
...
function formatProposal(
  ...
  return formatAdmissionNumber(context.format.pattern, {
    ...
    level: level ?? "",
    year:
      context.counter.resetFrequency === "calendar"
...
export const getAdmissionNumberPolicy = query({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      activeSessionId: context.session?._id ?? null,
      resetPeriod: context.period ?? null,
...
export async function proposeAdmissionNumberHelper(
  ...
  let sequenceNumber = context.sequence;
  let allocatedNumber = formatProposal(context, args.level, sequenceNumber);
  for (let attempts = 0; attempts < 1_000; attempts += 1) {
    const [student, claim] = await Promise.all([
      ctx.db.query("students")
        .withIndex("by_school_and_admission_number", (q) =>
          q.eq("schoolId", args.schoolId).eq("admissionNumber", allocatedNumber),
        )
        .first(),
      ctx.db.query("admissionNumberClaims")
        .withIndex("by_school_number", (q) =>
          q.eq("schoolId", args.schoolId).eq("number", allocatedNumber),
        )
        .unique(),
    ]);
    if (!student && !claim) break;
    sequenceNumber += 1;
    validateSequence(sequenceNumber);
    allocatedNumber = formatProposal(context, args.level, sequenceNumber);
    if (attempts === 999) {
      throw new ConvexError("No admission number is available within the supported allocation window");
    }
  }
  return {
    allocatedNumber,
    ...
    activeSessionId: context.session._id,
...
export async function commitManualAdmissionNumberHelper(
  ...
  if (args.counterDecision === "advance" && args.advanceTo !== undefined) {
    ...
    if (
      ...
      args.expectedResetPeriod === undefined
    ) {
      throw new ConvexError(
        "Explicit counter advancement requires reviewed format, counter, session, and reset-period versions",
      );
    }
  ...
  await recordAuditEventHelper(ctx, {
    ...
    outcome: "success",
    safeSummary: `Manual admission override. Reason: ${args.reason.trim()}. Counter '${context?.counter?.key ?? "unchanged"}': ${args.counterDecision === "keep" ? "unchanged" : `explicit next ${args.advanceTo}`}`,
    alertTier: "tier2_warn",
    retentionClass: "permanent_statutory",
  });
}
