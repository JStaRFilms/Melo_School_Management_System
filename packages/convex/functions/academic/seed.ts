...
export const inspectDemoAuthUsageInternal = internalQuery({
  args: { authIds: v.array(v.string()), emails: v.array(v.string()) },
  ...
  handler: async (ctx, args) => {
    ...
    for (const authId of args.authIds) {
      ...
      if (user) {
        ...
        if (school?.slug !== DEMO_SCHOOL_SLUG) conflicts.add(`auth id ${authId} is linked to ${school?.slug ?? "a missing school"}`);
      }
    ...
    for (const email of args.emails) {
      ...
      if (user) {
        const school = await ctx.db.get(user.schoolId);
        if (school?.slug !== DEMO_SCHOOL_SLUG) conflicts.add(`email ${email} is linked to ${school?.slug ?? "a missing school"}`);
      }
...
export const getPendingDemoStorageCleanupInternal = internalQuery({
  ...
  handler: async (ctx) => {
    const rows = await ctx.db.query("demoSeedStorageCleanup").withIndex("by_school_slug", (q) => q.eq("schoolSlug", DEMO_SCHOOL_SLUG)).take(75);
...
export const clearDemoSchoolBatchInternal = internalMutation({
  ...
  handler: async (ctx) => {
    const school = await ctx.db
      .query("schools")
      .withIndex("by_slug", (q) => q.eq("slug", DEMO_SCHOOL_SLUG))
      .unique();
    ...
    for (const tableName of DEMO_SCHOOL_TABLES) {
      ...
      await recordStorageCleanup(ctx, school._id, storageIds);
...
async function loadContext(ctx: MutationCtx, runId: Id<"demoSeedRuns">): Promise<SeedRunContext> {
  ...
  const adminUserId = byEmail.get("admin@demo-academy.school")?._id;
  const teacherIds = ["teacher@demo-academy.school", "sade.adeyemi@demo-academy.school", "chinedu.okoro@demo-academy.school"]
    .map((email) => byEmail.get(email)?._id);
  const portalUserId = byEmail.get("parent@demo-academy.school")?._id;
  ...
  if (!adminUserId || teacherIds.some((id) => !id) || !portalUserId || !sessionId || termIds.length !== 3 || classIds.length !== 3 || subjectIds.length !== DEMO_SUBJECTS.length || familyIds.length !== 18) {
...
export const startDemoSeedRunInternal = internalMutation({
  args: { adminAuthId: v.string(), teacherAuthId: v.string(), portalAuthId: v.string(), logoStorageId: v.id("_storage"), portraitStorageIds: v.array(v.id("_storage")) },
  returns: v.id("demoSeedRuns"),
  handler: async (ctx, args) => {
    if (args.portraitStorageIds.length !== DEMO_STUDENTS.length) throw new ConvexError(`Expected ${DEMO_STUDENTS.length} portrait PNG assets.`);
    const existing = await ctx.db.query("schools").withIndex("by_slug", (q) => q.eq("slug", DEMO_SCHOOL_SLUG)).unique();
    if (existing) throw new ConvexError("demo-school must be reset before a new seed run starts.");
    const schoolId = await ctx.db.insert("schools", { name: "Demo Academy", slug: DEMO_SCHOOL_SLUG, status: "active", logoStorageId: args.logoStorageId, logoFileName: "demo-academy-crest.png", logoContentType: "image/png", logoUpdatedAt: DEMO_CREATED_AT, createdAt: DEMO_CREATED_AT, updatedAt: DEMO_CREATED_AT });
    return await ctx.db.insert("demoSeedRuns", { schoolId, status: "running", phase: "foundation", studentCursor: 0, assessmentCursor: 0, billingCursor: 0, ...args, createdAt: Date.now(), updatedAt: Date.now() });
  },
});
...
export const populateDemoFoundationInternal = internalMutation({
  ...
  handler: async (ctx, { runId }) => {
    ...
    const now = DEMO_CREATED_AT; const schoolId = run.schoolId;
    const adminUserId = await ctx.db.insert("users", { schoolId, authId: run.adminAuthId, name: "Amina Okafor", firstName: "Amina", lastName: "Okafor", email: "admin@demo-academy.school", role: "admin", isSchoolAdmin: true, createdAt: now, updatedAt: now });
    const teacherUserId = await ctx.db.insert("users", { schoolId, authId: run.teacherAuthId, name: "Daniel Mensah", firstName: "Daniel", lastName: "Mensah", email: "teacher@demo-academy.school", role: "teacher", createdAt: now, updatedAt: now });
    const teacherTwoId = await ctx.db.insert("users", { schoolId, authId: "demo-teacher-sade-adeyemi", name: "Sade Adeyemi", firstName: "Sade", lastName: "Adeyemi", email: "sade.adeyemi@demo-academy.school", role: "teacher", createdAt: now, updatedAt: now });
    const teacherThreeId = await ctx.db.insert("users", { schoolId, authId: "demo-teacher-chinedu-okoro", name: "Chinedu Okoro", firstName: "Chinedu", lastName: "Okoro", email: "chinedu.okoro@demo-academy.school", role: "teacher", createdAt: now, updatedAt: now });
    const portalUserId = await ctx.db.insert("users", { schoolId, authId: run.portalAuthId, name: "Grace Adeyemi", firstName: "Grace", lastName: "Adeyemi", email: "parent@demo-academy.school", role: "parent", createdAt: now, updatedAt: now });
    ...
    const teachers = [teacherUserId, teacherTwoId, teacherThreeId]; const classIds: Id<"classes">[] = [];
    for (let index = 0; index < DEMO_CLASSES.length; index += 1) classIds.push(await ctx.db.insert("classes", { schoolId, ...DEMO_CLASSES[index], formTeacherId: teachers[index], isArchived: false, createdAt: now, updatedAt: now }));
    const subjectIds: Id<"subjects">[] = [];
    for (const [name, code] of DEMO_SUBJECTS) subjectIds.push(await ctx.db.insert("subjects", { schoolId, name, code, isArchived: false, createdAt: now, updatedAt: now }));
    ...
    for (const band of DEMO_BANDS) await ctx.db.insert("gradingBands", { schoolId, ...band, isActive: true, createdAt: now, updatedAt: now, updatedBy: adminUserId });
    ...
    await ctx.db.insert("schoolBillingSettings", { schoolId, invoicePrefix: "DA", defaultCurrency: "NGN", defaultDueDays: 30, preferredProvider: "manual", paymentProviderMode: "test", allowManualPayments: true, allowOnlinePayments: false, createdAt: now, updatedAt: now, updatedBy: adminUserId });
    for (let classIndex = 0; classIndex < classIds.length; classIndex += 1) { const feePlanId = await ctx.db.insert("feePlans", { schoolId, name: `${DEMO_CLASSES[classIndex].gradeName} Third Term Fees`, description: "Tuition, learning resources, and activities", currency: "NGN", billingMode: "class_default", targetClassIds: [classIds[classIndex]], lineItems: [{ id: "tuition", label: "Tuition", amount: 85000, category: "tuition", order: 0 }, { id: "learning", label: "Learning resources", amount: 12000, category: "activity", order: 1 }, { id: "assessment", label: "Assessment", amount: 5000, category: "exam", order: 2 }], installmentPolicy: { enabled: true, installmentCount: 2, intervalDays: 30, firstDueDays: 14 }, isActive: true, createdAt: now, updatedAt: now, createdBy: adminUserId, updatedBy: adminUserId }); await ctx.db.insert("feePlanApplications", { schoolId, feePlanId, classId: classIds[classIndex], sessionId, termId: termIds[2], studentCount: 12, createdInvoiceCount: 12, skippedInvoiceCount: 0, notes: "Deterministic demo application", createdAt: now, updatedAt: now, createdBy: adminUserId }); }
    for (const [title, description, location, date] of DEMO_EVENTS) await ctx.db.insert("schoolEvents", { schoolId, title, description, location, startDate: timestamp(date), endDate: timestamp(date) + DAY, isAllDay: true, isArchived: false, createdAt: now, updatedAt: now, updatedBy: adminUserId });
...
export const populateDemoStudentsBatchInternal = internalMutation({
  ...
  handler: async (ctx, { runId }) => {
    ...
    const data = await loadContext(ctx, runId); const end = Math.min(run.studentCursor + 12, DEMO_STUDENTS.length); const now = DEMO_CREATED_AT;
    for (let index = run.studentCursor; index < end; index += 1) { const student = DEMO_STUDENTS[index]; const [firstName, lastName] = student.name.split(" "); const userId = await ctx.db.insert("users", { schoolId: data.schoolId, authId: `demo-student-${student.admissionNumber}`, name: student.name, firstName, lastName, email: student.email, role: "student", createdAt: now, updatedAt: now }); const studentId = await ctx.db.insert("students", { schoolId: data.schoolId, classId: data.classIds[student.classIndex], userId, familyId: data.familyIds[student.familyIndex], admissionNumber: student.admissionNumber, houseName: ["Courage", "Unity", "Integrity", "Discovery"][index % 4], gender: student.gender, dateOfBirth: timestamp(`201${index % 4 + 1}-0${index % 8 + 1}-15`), guardianName: student.familyIndex === 0 ? "Grace Adeyemi" : `Parent ${student.familyIndex + 1} Demo`, guardianPhone: `+234 800 555 ${String(1000 + index)}`, address: `${index + 1} Learning Lane, Demo City`, photoStorageId: run.portraitStorageIds[index], photoFileName: `portrait-${String(index + 1).padStart(2, "0")}.png`, photoContentType: "image/png", photoUpdatedAt: now, isArchived: false, createdAt: now, updatedAt: now }); for (const subjectId of data.subjectIds) await ctx.db.insert("studentSubjectSelections", { schoolId: data.schoolId, studentId, classId: data.classIds[student.classIndex], subjectId, sessionId: data.sessionId, createdAt: now, updatedAt: now }); }
    const phase: "students" | "assessments" = end === DEMO_STUDENTS.length ? "assessments" : "students"; await updateRunPhase(ctx, runId, phase, { studentCursor: end, assessmentCursor: phase === "assessments" ? 0 : run.assessmentCursor }); return { phase, cursor: end };
  },
});
...
export const populateDemoBillingBatchInternal = internalMutation({
  args: { runId: v.id("demoSeedRuns") }, returns: v.object({ phase: runPhaseValidator, cursor: v.number() }),
  handler: async (ctx, { runId }) => {
    const run = await requireRun(ctx, runId); if (run.phase !== "billing") return { phase: run.phase, cursor: run.billingCursor }; const data = await loadContext(ctx, runId); const students = await ctx.db.query("students").withIndex("by_school", (q) => q.eq("schoolId", data.schoolId)).take(50); const feePlanIds = await ctx.db.query("feePlans").withIndex("by_school", (q) => q.eq("schoolId", data.schoolId)).take(5).then((rows) => rows.map((row) => row._id)); const applicationIds = await ctx.db.query("feePlanApplications").withIndex("by_school", (q) => q.eq("schoolId", data.schoolId)).take(5).then((rows) => rows.map((row) => row._id)); if (feePlanIds.length !== 3 || applicationIds.length !== 3) throw new ConvexError("Missing billing foundation."); const end = Math.min(run.billingCursor + 12, students.length); const now = DEMO_CREATED_AT;
    for (let index = run.billingCursor; index < end; index += 1) { const classIndex = DEMO_STUDENTS[index].classIndex; const totalAmount = 102000; const amountPaid = index % 5 === 0 ? totalAmount : index % 3 === 0 ? 51000 : 0; const status = amountPaid === totalAmount ? "paid" : amountPaid > 0 ? "partially_paid" : index % 4 === 0 ? "overdue" : "issued" as const; const invoiceId = await ctx.db.insert("studentInvoices", { schoolId: data.schoolId, feePlanId: feePlanIds[classIndex], feePlanApplicationId: applicationIds[classIndex], studentId: students[index]._id, classId: data.classIds[classIndex], sessionId: data.sessionId, termId: data.termIds[2], invoiceNumber: `DA-2026-3-${String(index + 1).padStart(3, "0")}`, feePlanNameSnapshot: `${DEMO_CLASSES[classIndex].gradeName} Third Term Fees`, currency: "NGN", lineItems: [{ id: "tuition", label: "Tuition", amount: 85000, category: "tuition", order: 0 }, { id: "learning", label: "Learning resources", amount: 12000, category: "activity", order: 1 }, { id: "assessment", label: "Assessment", amount: 5000, category: "exam", order: 2 }], installmentSchedule: [{ id: "installment-1", label: "First installment", dueAt: timestamp("2026-05-15"), amount: 51000, isPaid: amountPaid >= 51000 }, { id: "installment-2", label: "Final installment", dueAt: timestamp("2026-06-15"), amount: 51000, isPaid: amountPaid === totalAmount }], subtotal: totalAmount, waiverAmount: 0, discountAmount: 0, totalAmount, amountPaid, balanceDue: totalAmount - amountPaid, status, dueDate: timestamp("2026-06-15"), issuedAt: timestamp("2026-04-20"), issuedBy: data.adminUserId, notes: "Demo invoice — recorded payments are manual only.", createdAt: now, updatedAt: now }); if (amountPaid > 0) { const paymentId = await ctx.db.insert("billingPayments", { schoolId: data.schoolId, invoiceId, reference: `DEMO-MANUAL-${String(index + 1).padStart(3, "0")}`, provider: "manual", paymentMethod: index % 2 ? "bank_transfer" : "cash", amountReceived: amountPaid, amountApplied: amountPaid, unappliedAmount: 0, applicationStatus: "applied", status: "successful", payerName: index < 2 ? "Grace Adeyemi" : `Parent ${DEMO_STUDENTS[index].familyIndex + 1} Demo`, payerEmail: index < 2 ? "parent@demo-academy.school" : undefined, receivedAt: timestamp("2026-05-20") + index * DAY, recordedBy: data.adminUserId, reconciliationStatus: "reconciled", reconciledBy: data.adminUserId, reconciledAt: now, notes: "Seeded manual payment; no gateway transaction.", createdAt: now, updatedAt: now }); await ctx.db.insert("paymentAllocations", { schoolId: data.schoolId, invoiceId, paymentId, amountApplied: amountPaid, createdAt: now, createdBy: data.adminUserId }); await ctx.db.patch(invoiceId, { lastPaymentId: paymentId, lastPaymentAt: timestamp("2026-05-20") + index * DAY }); } }
...
export const populateDemoKnowledgeAndFinalizeInternal = internalMutation({
  ...
  handler: async (ctx, { runId }) => {
    const run = await requireRun(ctx, runId); if (run.phase === "complete") return { schoolId: run.schoolId, studentCount: DEMO_STUDENTS.length, classCount: DEMO_CLASSES.length, invoiceCount: DEMO_STUDENTS.length, assessmentRecordCount: DEMO_STUDENTS.length * DEMO_SUBJECTS.length * 3 }; if (run.phase !== "knowledge") throw new ConvexError(`Cannot finalize demo seed from ${run.phase}.`); const data = await loadContext(ctx, runId); const now = DEMO_CREATED_AT; const topicIds: Id<"knowledgeTopics">[] = [];
