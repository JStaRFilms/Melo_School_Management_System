...
function groupSourceQuery(
  ctx: Context,
  source: GroupAuditSource,
  filters: Filters,
  boundary?: { kind: "same_timestamp" | "older"; cursor: GroupCursor },
) {
  if (source.kind === "snapshot") {
    return ctx.db.query("auditEvents").withIndex("by_group_timestamp_eventId", q => {
      const group = q.eq("groupId", source.groupId);
      if (boundary?.kind === "same_timestamp")
        return group.eq("timestamp", boundary.cursor.timestamp).lt("eventId", boundary.cursor.eventId);
      if (boundary?.kind === "older")
        return filters.startDate === undefined
          ? group.lt("timestamp", boundary.cursor.timestamp)
          : group.gte("timestamp", filters.startDate).lt("timestamp", boundary.cursor.timestamp);
  ...
  return ctx.db.query("auditEvents").withIndex("by_school_group_timestamp_eventId", q => {
    const legacy = q.eq("schoolId", source.schoolId).eq("groupId", undefined);
    if (boundary?.kind === "same_timestamp")
      return legacy.eq("timestamp", boundary.cursor.timestamp).lt("eventId", boundary.cursor.eventId);
    if (boundary?.kind === "older")
      return filters.startDate === undefined
        ? legacy.lt("timestamp", boundary.cursor.timestamp)
        : legacy.gte("timestamp", filters.startDate).lt("timestamp", boundary.cursor.timestamp);
...
function encodeGroupCursor(cursor: GroupCursor) {
  return JSON.stringify({ version: 4, ...cursor });
}

function decodeGroupCursor(
  raw: string,
  sources: GroupAuditSource[],
): GroupCursor {
  let parsed: unknown;
  try {
  ...
  if (
    ...
    !("sourceKeys" in parsed) ||
    !Array.isArray(parsed.sourceKeys) ||
    parsed.sourceKeys.some((key) => typeof key !== "string")
  ) {
    throw new ConvexError("Invalid group audit cursor");
  }
  if (
    parsed.sourceKeys.length !== sources.length ||
    parsed.sourceKeys.some((key, index) => key !== sources[index].key)
  ) {
    throw new ConvexError("Group audit scope changed; restart pagination");
  }
  return {
...
async function groupAuditPage(
  ctx: Context,
  args: Filters & {
    scope: { kind: "group"; groupId: Id<"schoolGroups"> };
  ...
  const cursor = args.paginationOpts.cursor
    ? decodeGroupCursor(args.paginationOpts.cursor, sources)
    : null;
  const numItems = Math.min(Math.max(args.paginationOpts.numItems, 1), 100);
  const sourcePages = await Promise.all(
    sources.map(async (source) => {
      if (!cursor)
        return groupSourceQuery(ctx, source, args).order("desc").take(numItems + 1);
      const [sameTimestamp, older] = await Promise.all([
        groupSourceQuery(ctx, source, args, { kind: "same_timestamp", cursor }).order("desc").take(numItems + 1),
        groupSourceQuery(ctx, source, args, { kind: "older", cursor }).order("desc").take(numItems + 1),
      ]);
      return [...sameTimestamp, ...older].sort((left, right) =>
        right.timestamp - left.timestamp || right.eventId.localeCompare(left.eventId),
      ).slice(0, numItems + 1);
    }),
  );
  const merged = sourcePages.flat().sort((left, right) =>
    right.timestamp - left.timestamp || right.eventId.localeCompare(left.eventId),
  );
...
async function auditPage(
  ...
  if (args.scope.kind === "group" && !args.branchId) {
    ...
    const source = schoolId
      ? args.scope.kind === "platform"
        ? ctx.db.query("auditEvents").withIndex("by_school_and_actorKind_and_timestamp", (q) => {
            const branch = q.eq("schoolId", schoolId).eq("actorKind", "platform_admin");
      ...
      : args.scope.kind === "platform"
        ? ctx.db.query("auditEvents").withIndex("by_actorKind_and_timestamp", (q) => {
            const platform = q.eq("actorKind", "platform_admin");
            if (args.startDate !== undefined && args.endDate !== undefined) return platform.gte("timestamp", args.startDate).lte("timestamp", args.endDate);
            if (args.startDate !== undefined) return platform.gte("timestamp", args.startDate);
            if (args.endDate !== undefined) return platform.lte("timestamp", args.endDate);
            return platform;
          })
