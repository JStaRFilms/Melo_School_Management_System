...
function isAnchoredAnnualPeriod(contractStart: number, periodStart: number, periodEnd: number): boolean {
  let boundary = contractStart;
  for (let year = 0; year <= 100 && boundary <= periodStart; year += 1) {
    if (boundary === periodStart) return annualAnniversary(boundary) === periodEnd;
    boundary = annualAnniversary(boundary);
  }
  return false;
}
...
export const issueSubscriptionInvoice = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (
      ...
      !isAnchoredAnnualPeriod(contract.effectiveFrom, args.periodStart, args.periodEnd)
    )
      throw new ConvexError(
        "No-proration annual invoices must match a full cadence period anchored to the contract start",
      );
...
const GROUP_COMMERCIAL_BRANCH_LIMIT = 3;
...
export const getGroupCommercialSummary = query({
  ...
  handler: async (ctx, args) => {
    const links = await ctx.db
      ...
      .take(GROUP_COMMERCIAL_BRANCH_LIMIT + 1);
    if (links.length > GROUP_COMMERCIAL_BRANCH_LIMIT)
      throw new ConvexError(
        `Group commercial summary supports at most ${GROUP_COMMERCIAL_BRANCH_LIMIT} branches per request; use branch workspaces for larger groups`,
      );
