    ...
    for (const membership of rows) {
      ...
      if (user.isArchived) {
        if (user.role !== "student")
          throw identityError("FORBIDDEN", "Portal account is archived");
        const enrollment = await ctx.db.query("students")
          .withIndex("by_school_and_user", (q) =>
            q.eq("schoolId", user.schoolId).eq("userId", user._id),
          )
          .unique();
        if (enrollment?.enrollmentStatus !== "transferred_out")
          throw identityError("FORBIDDEN", "Archived student account has no transferred enrollment history");
      }
