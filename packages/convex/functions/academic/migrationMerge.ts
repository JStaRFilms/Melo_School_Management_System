  ...
  handler: async (ctx, args) => {
    ...
    for (const record of page.page) {
      ...
      if (usesOfficialCounter) {
        ...
        if (
          !selectedClass ||
          selectedClass.schoolId !== args.schoolId ||
          selectedClass.isArchived
        )
          throw new ConvexError(
            `Row #${record.rowNumber} class is archived or outside this school`,
          );
        ...
        if (
          ...
          record.expectedNumberSessionId !== counterState.sessionId ||
          record.expectedNumberResetPeriod !== counterState.resetPeriod
        ) {
          throw new ConvexError(
      ...
      if (record.entityType === "student") {
        ...
        if (record.admissionNumberMode === "official_generated") {
          const allocation = await allocateNextAdmissionNumberHelper(ctx, {
            ...
            expectedSessionId: record.expectedNumberSessionId,
          ...
          await commitManualAdmissionNumberHelper(ctx, {
            ...
            counterDecision:
              record.advanceCounterTo === undefined ? "keep" : "advance",
