...
export const queueKnowledgeMaterialOcrInternal = internalMutation({
  args: {
    materialId: v.id("knowledgeMaterials"),
    schoolId: v.id("schools"),
    actorUserId: v.id("users"),
  ...
  handler: async (ctx, args) => {
    const material = await ctx.db.get(args.materialId);
    if (!material || material.schoolId !== args.schoolId || !material.storageId) {
      return null;
    }
    ...
    const processingJob = processingJobs.find((job) => job.storageId === material.storageId);
    if (processingJob) {
      return { jobId: processingJob._id };
    }
    ...
    await ctx.runMutation(internal.functions.academic.lessonKnowledgeIngestion.recordContentAuditEventInternal, {
      ...
      entityType: "knowledgeMaterial",
      materialId: args.materialId,
      changeSummary: "Auto-queued provider-backed OCR for scanned or image-heavy file.",
    });
