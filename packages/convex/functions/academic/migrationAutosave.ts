...
async function resolveGradeImportEvidence(
  ...
  const [student, subject, session, term] = await Promise.all([
    ctx.db.get(selectedStudentId),
    ctx.db.get(selectedSubjectId),
  ...
  if (!subject || subject.schoolId !== schoolId || subject.isArchived) {
    throw new ConvexError(`Grade row #${record.rowNumber} subject is unavailable in this school`);
  }
...
export async function validateReviewedRecord(
  ...
  if (record.entityType === "student") {
    if (action === "merge_existing") {
      ...
      if (
        target.isArchived ||
        (target.enrollmentStatus !== undefined &&
          target.enrollmentStatus !== "active")
      )
        throw new ConvexError(
          `Row #${record.rowNumber} merge target is inactive`,
        );
    ...
    if (suppliedNumber) {
      ...
      if (record.advanceCounterTo !== undefined) {
        ...
        if (
          ...
          record.expectedNumberSessionId === undefined ||
          !record.expectedNumberResetPeriod
        ) {
          throw new ConvexError(
  ...
  if (!selectedClass || selectedClass.schoolId !== schoolId || selectedClass.isArchived)
    throw new ConvexError(
      `Grade row #${record.rowNumber} class is outside this school or archived`,
    );
...
export const patchStagedRecord = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (workspace.status !== "committing") assertEditable(workspace);
    if (record.isCommitted)
      throw new ConvexError(
        "Staged record is already committed; committed row outcomes are immutable",
      );
    ...
    await ctx.db.patch(record._id, {
      ...
      expectedNumberSessionId: undefined,
      expectedNumberResetPeriod: undefined,
...
export const reviewStagedRecord = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const candidate: Doc<"stagedImportRecords"> = {
      ...
      expectedNumberSessionId: args.expectedNumberSessionId,
      expectedNumberResetPeriod: args.expectedNumberResetPeriod,
    ...
    if (
      ...
      if (candidate.advanceCounterTo !== undefined) {
        ...
        if (
          ...
          candidate.expectedNumberSessionId !== proposal.activeSessionId ||
          candidate.expectedNumberResetPeriod !== proposal.resetPeriod ||
    ...
    await ctx.db.patch(record._id, {
      ...
      expectedNumberSessionId: candidate.expectedNumberSessionId,
      expectedNumberResetPeriod: candidate.expectedNumberResetPeriod,
      proposedAdmissionNumber: undefined,
