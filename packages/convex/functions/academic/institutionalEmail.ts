...
async function visibleMailboxes(ctx: Context, schoolId: Id<"schools">) {
  ...
  const [pages, legacy] = await Promise.all([
    Promise.all(allowedKinds.map(recipientKind =>
    ...
    ctx.db.query("institutionalMailboxes")
      .withIndex("by_school_kind_and_email", q => q.eq("schoolId", schoolId).eq("recipientKind", undefined))
      .take(101),
  ]);
  if (pages.some(page => page.length > 100) || legacy.length > 100)
    throw new ConvexError("Mailbox compatibility query exceeds its supported 100-row recipient partition; use paginated discovery");
  const authorizedLegacy = [];
  for (const mailbox of legacy) {
    const kind = await targetKind(ctx, mailbox.personId, schoolId);
    if (kind !== "unclassified" && allowedKinds.includes(kind))
      authorizedLegacy.push({ ...mailbox, recipientKind: kind });
  }
  return [...pages.flat(), ...authorizedLegacy]
...
export const listInstitutionalMailboxesPage = query({
  ...
  handler: async (ctx, args) => {
    validatePageSize(args.paginationOpts.numItems);
    const { permissions } = await emailAccess(ctx, args.schoolId);
    if (!args.recipientKind) {
      if (!permissions.student || (!permissions.staff && !permissions.lifecycle))
        throw new ConvexError("Specify a recipient kind for scoped mailbox pagination");
      ...
      for (const mailbox of result.page) {
        const kind = mailbox.recipientKind ?? await targetKind(ctx, mailbox.personId, args.schoolId);
        if (kind !== "unclassified") page.push(mailboxMetadata(mailbox, kind));
      }
      return { ...result, page };
    }
    const recipientKind = args.recipientKind;
    const visible = recipientKind === "student"
      ? permissions.student
      : permissions.staff || permissions.lifecycle;
    if (!visible) throw new ConvexError("Institutional mailbox recipient scope required");
    const result = await ctx.db.query("institutionalMailboxes")
      .withIndex("by_school_kind_and_email", q =>
        q.eq("schoolId", args.schoolId).eq("recipientKind", recipientKind))
      .paginate(args.paginationOpts);
    return { ...result, page: result.page.map(mailbox => mailboxMetadata(mailbox, recipientKind)) };
  },
});
...
export const listLegacyInstitutionalMailboxesPage = query({
  ...
  handler: async (ctx, args) => {
    ...
    const result = await ctx.db.query("institutionalMailboxes")
      .withIndex("by_school_kind_and_email", q =>
        q.eq("schoolId", args.schoolId).eq("recipientKind", undefined))
      .paginate(args.paginationOpts);
    const page = [];
    for (const mailbox of result.page) {
...
export const listEmailDomainsPage = query({
  args: { schoolId: v.id("schools"), scope: v.literal("owned"), paginationOpts: paginationOptsValidator },
  handler: async (ctx, args) => {
    validatePageSize(args.paginationOpts.numItems);
    await emailAccess(ctx, args.schoolId);
    const result = await ctx.db.query("schoolEmailDomains")
    ...
    return { ...result, page: result.page.map(domain => domainMetadata(domain)) };
  },
});

export const listSharedEmailDomainsPage = query({
  args: {
    schoolId: v.id("schools"),
    ownerRange: v.union(v.literal("before"), v.literal("after")),
    paginationOpts: paginationOptsValidator,
  },
  handler: async (ctx, args) => {
    validatePageSize(args.paginationOpts.numItems);
    await emailAccess(ctx, args.schoolId);
    const link = await ctx.db.query("schoolGroupBranches")
      .withIndex("by_school", q => q.eq("schoolId", args.schoolId)).unique();
    const group = link ? await ctx.db.get(link.groupId) : null;
    if (group?.status !== "active")
      throw new ConvexError("Active group required for shared domain pagination");
    const result = await ctx.db.query("schoolEmailDomains")
      .withIndex("by_sharedGroupId_and_school_and_domain", q => {
        const owners = q.eq("sharedGroupId", group._id);
        return args.ownerRange === "before"
          ? owners.lt("schoolId", args.schoolId)
          : owners.gt("schoolId", args.schoolId);
      })
      .paginate(args.paginationOpts);
    return { ...result, page: result.page.map(domain => domainMetadata(domain, group._id)) };
  },
});
...
export const getSchoolEmailDomains = query({
  args: { schoolId: v.id("schools") }, handler: async (ctx, args) => {
    ...
    const domains = await ctx.db.query("schoolEmailDomains")
      .withIndex("by_school_and_domain", q => q.eq("schoolId", args.schoolId))
      .take(101);
    if (domains.length > 100) throw new ConvexError("Branch exceeds the supported 100-domain compatibility window; use paginated discovery");
...
export const getEmailWorkbench = query({
  args: { schoolId: v.id("schools") }, handler: async (ctx, args) => {
    ...
    if (policy?.domainId) {
      try {
        policyDomain = domainMetadata(await resolveDomain(ctx, args.schoolId, policy.domainId), group?._id);
      } catch {
        const configuredDomain = await ctx.db.get(policy.domainId);
        if (configuredDomain) policyDomain = domainMetadata(configuredDomain, group?._id);
        policyDomainUnavailable = true;
      }
    }
    return { permissions, policy, policyDomain, groupName: group?.status === "active" ? group.name : null,
