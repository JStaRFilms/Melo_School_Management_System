...
- set `LEGACY_SUBJECT_FALLBACK_ENABLED=true` on Convex only while same-deployment legacy `authId` compatibility is required. Alternatively, set `LEGACY_SUBJECT_TRUSTED_ISSUER` to one exact Better Auth JWT issuer URL. Leaving both unset disables subject fallback.
