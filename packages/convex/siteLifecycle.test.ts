...
const siteApi = api as unknown as { functions: { foundation: {
  ...
  siteDomains: {
    ...
    setCanonicalDomain: FunctionReference<"mutation", "public", { schoolId: Id<"schools">; domainId: Id<"schoolDomains"> }, null>;
  };
...
const internalSiteApi = internal as unknown as { functions: { foundation: { siteDomains: { advanceDomainLifecycleInternal: FunctionReference<"mutation", "internal", { domainId: Id<"schoolDomains">; nextStatus: "verified" | "routing_pending" | "certificate_pending" | "ready" | "active"; verificationToken?: string }, null> } } } };
...
async function fixture(t: ReturnType<typeof convexTest>, slug = "alpha") {
  ...
  return await t.run(async (ctx) => {
    ...
    for (const capability of ["site.publish.standard", "site.preview", "site.domain.request"] as const) await ctx.db.insert("schoolCapabilityGrants", { schoolId, userId, capability, scope: "school", grantedByUserId: userId, reason: "test", isBreakGlass: false, createdAt: now });
...
describe("B4/B5 managed publication controls", () => {
  test("bootstraps a managed initial draft and enforces the bounded domain lifecycle, tenant hostname ownership, and one canonical host", async () => {
    ...
    await t.withIdentity(first.identity).mutation(siteApi.functions.foundation.siteDomains.setCanonicalDomain, { schoolId: first.schoolId, domainId: firstDomain });
  ...
  test("publishes only approved renderer fields with exact evidence and projects an immutable revision", async () => {
    ...
    await t.withIdentity(school.identity).mutation(siteApi.functions.foundation.siteDomains.setCanonicalDomain, { schoolId: school.schoolId, domainId });
    ...
    const evidenceIds = await t.run(async (ctx) => {
      ...
      ids.push(await ctx.db.insert("schoolApprovalEvidence", { schoolId: school.schoolId, approvalClass: "identity", subjectType: "site_field", subjectKey: "identity.displayName", evidenceReference: "test", approvedValueDigest: await hash(JSON.stringify(content.fields[0]!.value)), approvedAt: now, createdAt: now }));
      for (const seo of content.routeSeo) ids.push(await ctx.db.insert("schoolApprovalEvidence", { schoolId: school.schoolId, approvalClass: "standard", subjectType: "site_route_seo", subjectKey: seo.routeId, evidenceReference: "test", approvedValueDigest: await hash(JSON.stringify({ title: seo.title, description: seo.description, shareAssetId: null })), approvedAt: now, createdAt: now }));
      return ids;
    });
  ...
  test("rejects a field asset that lacks tenant-scoped rights evidence, approved purpose/channel, and field-use eligibility", async () => {
    ...
    const evidenceIds = await t.run(async (ctx) => {
      const ids: Id<"schoolApprovalEvidence">[] = [];
      for (const field of content.fields) ids.push(await ctx.db.insert("schoolApprovalEvidence", { schoolId: school.schoolId, approvalClass: field.fieldId.startsWith("identity") ? "identity" : "standard", subjectType: "site_field", subjectKey: field.fieldId, evidenceReference: "test", approvedValueDigest: await hash(JSON.stringify(field.value)), approvedAt: now, createdAt: now }));
      for (const seo of content.routeSeo) ids.push(await ctx.db.insert("schoolApprovalEvidence", { schoolId: school.schoolId, approvalClass: "standard", subjectType: "site_route_seo", subjectKey: seo.routeId, evidenceReference: "test", approvedValueDigest: await hash(JSON.stringify({ title: seo.title, description: seo.description, shareAssetId: null })), approvedAt: now, createdAt: now }));
      return ids;
    });
    ...
    await expect(t.withIdentity(school.identity).mutation(siteApi.functions.foundation.siteLifecycle.publishDraft, { schoolId: school.schoolId, expectedDraftVersion: 2 })).rejects.toThrow("Current exact approval evidence is required for publication");
  });
