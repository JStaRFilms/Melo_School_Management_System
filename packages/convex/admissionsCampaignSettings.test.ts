...
describe("atomic admissions campaign commands", () => {
  ...
  test("requires explicit intent to recover the exact unused open partial as a draft and preserves its evidence", async () => {
    ...
    const before = await t.run(async (ctx) => ({ intake: await ctx.db.get(legacy.intakeId), form: await ctx.db.get(legacy.formId), declaration: await ctx.db.get(legacy.declarationId), price: await ctx.db.get(legacy.priceId), fields: await ctx.db.query("admissionsFormFields").withIndex("by_form_version_and_order", (q) => q.eq("formVersionId", legacy.formId)).take(10), requirements: await ctx.db.query("admissionsDocumentRequirements").withIndex("by_form_version_and_order", (q) => q.eq("formVersionId", legacy.formId)).take(10), forms: await ctx.db.query("admissionsFormVersions").withIndex("by_intake", (q) => q.eq("intakeId", legacy.intakeId)).take(10), declarations: await ctx.db.query("admissionsDeclarationVersions").withIndex("by_school_and_programme_and_version", (q) => q.eq("schoolId", schoolId).eq("programmeId", legacy.programmeId).gte("version", 0)).take(10), prices: await ctx.db.query("admissionsProductPrices").withIndex("by_product_and_version", (q) => q.eq("productId", legacy.productId)).take(10), operations: await ctx.db.query("admissionsCampaignOperations").withIndex("by_intake", (q) => q.eq("intakeId", legacy.intakeId)).take(10), audits: await ctx.db.query("admissionsAuditEvents").withIndex("by_school_and_created_at", (q) => q.eq("schoolId", schoolId)).take(20) }));
    ...
    expect(after.intake).toEqual({ ...before.intake, status: "draft" }); expect(after.product).toMatchObject({ status: "draft" }); expect(after.form).toEqual(before.form); expect(after.declaration).toEqual(before.declaration); expect(after.price).toEqual(before.price); expect(after.fields).toEqual(before.fields); expect(after.requirements).toEqual(before.requirements); expect(after.forms).toHaveLength(before.forms.length + 1); expect(after.declarations).toHaveLength(before.declarations.length + 1); expect(after.prices).toEqual(before.prices); expect(after.operations).toHaveLength(before.operations.length + 1); expect(after.audits).toHaveLength(before.audits.length + 1); expect(after.audits).toEqual(expect.arrayContaining([expect.objectContaining({ action: "catalogue.campaign_legacy_recovered_to_draft", actorUserId: userId, reasonCode: "legacy_recovery_to_draft" })])); expect(await t.run((ctx) => ctx.db.get(recovered.formVersionId))).toMatchObject({ status: "draft", version: 2 }); expect(await t.run((ctx) => ctx.db.get(recovered.declarationVersionId))).toMatchObject({ status: "draft", version: 2 });
  });

  test("requires exactly one v1 published form, declaration, and price before recovering", async () => {
    const t = convexTest(schema, modules); const { schoolId } = await fixture(t); const now = Date.now();
    const extraForm = await insertRecoverableLegacyPartial(t, schoolId);
    const extraDeclaration = await insertRecoverableLegacyPartial(t, schoolId);
    const extraPrice = await insertRecoverableLegacyPartial(t, schoolId);
    await t.run(async (ctx) => {
      await ctx.db.insert("admissionsFormVersions", { schoolId, programmeId: extraForm.programmeId, intakeId: extraForm.intakeId, version: 2, schemaVersion: "1", legalNamePolicyVersion: 2, status: "draft", createdAt: now, updatedAt: now });
      await ctx.db.insert("admissionsDeclarationVersions", { schoolId, programmeId: extraDeclaration.programmeId, version: 2, title: "Extra declaration", body: "Extra declaration", bodyDigest: "extra-declaration", purpose: "service", status: "retired", createdAt: now, updatedAt: now });
      await ctx.db.insert("admissionsProductPrices", { schoolId, productId: extraPrice.productId, version: 2, amountMinor: 10_000, currency: "NGN", refundPolicyKey: "non_refundable", feeDisclosure: "Fee", effectiveFrom: now, status: "draft", createdAt: now, updatedAt: now });
    });
    const projection = await t.withIdentity(editor).query(api.functions.admissions.settings.listLegacyCampaignRecovery, { schoolId });
    for (const intakeId of [extraForm.intakeId, extraDeclaration.intakeId, extraPrice.intakeId]) expect(projection).toEqual(expect.arrayContaining([expect.objectContaining({ intakeId, recoveryState: "review_required" })]));
    for (const [intakeId, operationKey] of [[extraForm.intakeId, "recover-extra-form"], [extraDeclaration.intakeId, "recover-extra-declaration"], [extraPrice.intakeId, "recover-extra-price"]] as const) await expect(t.withIdentity(editor).mutation(api.functions.admissions.settings.replaceCampaignConfiguration, { schoolId, intakeId, operationKey, targetStatus: "draft", recoverLegacyToDraft: true, configuration: replaceConfiguration(now) })).rejects.toThrow("RECOVERY_GRAPH_AMBIGUOUS");
    expect(await t.run(async (ctx) => (await ctx.db.query("admissionsCampaignOperations").take(10)).filter((row) => [extraForm.intakeId, extraDeclaration.intakeId, extraPrice.intakeId].includes(row.intakeId)))).toEqual([]);
  });

  test("rejects cross-school children attached to the sole recovery form", async () => {
    const t = convexTest(schema, modules); const { schoolId } = await fixture(t); const legacy = await insertRecoverableLegacyPartial(t, schoolId); const now = Date.now();
    await t.run(async (ctx) => {
      const foreignSchoolId = await ctx.db.insert("schools", { name: "Foreign", slug: `foreign-child-${now}`, status: "active", createdAt: now, updatedAt: now });
      await ctx.db.insert("admissionsFormFields", { schoolId: foreignSchoolId, formVersionId: legacy.formId, fieldKey: "foreign-field", sectionKey: "applicant", kind: "text", label: "Foreign field", requiredMode: "optional", dataClass: "personal", validationJson: "{}", order: 99, status: "active", createdAt: now, updatedAt: now });
      await ctx.db.insert("admissionsDocumentRequirements", { schoolId: foreignSchoolId, formVersionId: legacy.formId, requirementKey: "foreign-requirement", category: "identity", label: "Foreign requirement", requiredMode: "required", acceptedMimeTypes: ["application/pdf"], maxBytes: 1_000, maxFiles: 1, sensitivity: "child_confidential", purpose: "Identity", order: 99, createdAt: now, updatedAt: now });
    });
    const projection = await t.withIdentity(editor).query(api.functions.admissions.settings.listLegacyCampaignRecovery, { schoolId });
    expect(projection).toEqual(expect.arrayContaining([expect.objectContaining({ intakeId: legacy.intakeId, recoveryState: "review_required" })]));
    await expect(t.withIdentity(editor).mutation(api.functions.admissions.settings.replaceCampaignConfiguration, { schoolId, intakeId: legacy.intakeId, operationKey: "recover-cross-school-child", targetStatus: "draft", recoverLegacyToDraft: true, configuration: replaceConfiguration(now) })).rejects.toThrow("RECOVERY_GRAPH_AMBIGUOUS");
    expect(await t.run((ctx) => ctx.db.query("admissionsCampaignOperations").withIndex("by_intake", (q) => q.eq("intakeId", legacy.intakeId)).take(2))).toEqual([]);
  });
