...
const mocks = vi.hoisted(() => ({ query: vi.fn(), replace: vi.fn(), push: vi.fn(), signOut: vi.fn(), departure: vi.fn() }));
...
describe("default-school shell", () => {
  ...
  it("switches only on a scoped route after guard approval and scopes branding to the target", async () => {
    ...
    access = {
      ...
      membership: { membershipId: "membership-two", personId: "person", displayTitle: "Admin", isProprietor: false },
