...
vi.mock("convex/react", () => ({
  usePaginatedQuery: () => ({
    results: [
      {
        ...record,
        reviewStatus: mocks.reviewed ? "approved" : "pending",
        validationStatus: mocks.reviewed ? "valid" : "warning",
        isResolved: mocks.reviewed,
      },
    ],
    status: "Exhausted",
    loadMore: vi.fn(),
  ...
  useQuery: (name: string, args: unknown) => {
    ...
    if (name.endsWith(":listWorkspaces"))
      return [
        {
          _id: "workspace",
          name: "Private import",
    ...
    if (name.endsWith(":getWorkspaceSummary"))
      return {
        name: "Private import",
        status: mocks.status,
        totalRecords: 1,
        validRecords: mocks.reviewed ? 1 : 0,
        errorRecords: 0,
        warningRecords: mocks.reviewed ? 0 : 1,
      };
    if (name.endsWith(":getWorkspaceReviewOptions"))
      return {
        classes: [{ id: "class", name: "JSS 1A", level: "JSS 1" }],
        subjects: [],
        families: [],
        students: [],
        availableStudentUsers: [{ id: "user", name: "Ada Identity" }],
        sessions: [],
        numbering: {
          available: true,
          nextNumber: "SCH/2026/0010",
          nextSequence: 10,
          policyVersion: 1,
          formatVersion: "branch:1:0",
          counterKey: "default",
          counterVersion: 0,
        },
      };
...
afterEach(() => {
  ...
  mocks.status = "reviewing";
  mocks.reviewed = false;
});
...
it("truthfully gates commit until explicit row review and plan approval", async () => {
  ...
  expect(
    screen
      .getByRole("button", { name: "Commit approved plan" })
      .hasAttribute("disabled"),
  ).toBe(true);
  expect(
    screen
      .getByRole("button", { name: "Approve reviewed plan" })
      .hasAttribute("disabled"),
  ).toBe(true);
