...
vi.mock("convex/react", () => ({
  ...
  usePaginatedQuery: (reference: Parameters<typeof getFunctionName>[0], args: unknown) => {
    ...
    if (name.endsWith("listEmailDomainsPage") || name.endsWith("listSharedEmailDomainsPage")) {
      const scope = name.endsWith("listSharedEmailDomainsPage") ? "shared" : "owned";
      const results = mocks.empty
        ? []
        : scope === "shared"
          ? mocks.inheritedOnly
      ...
      return { results, status: mocks.pageStatus, loadMore: mocks.loadMore };
    }
    ...
    if (name.endsWith("listLegacyInstitutionalMailboxesPage") ||
        (typeof args === "object" && args !== null && "recipientKind" in args && args.recipientKind === "staff"))
      return { results: [], status: mocks.pageStatus, loadMore: mocks.loadMore };
...
beforeEach(() => {
  mocks.allowed = true; mocks.loading = false; mocks.empty = false; mocks.inheritedOnly = false; mocks.policy = true; mocks.version = 1; mocks.candidate = "ada.example2"; mocks.pageStatus = "Exhausted"; mocks.draft = null;
...
it("loads bounded cursor pages explicitly for domains, proposal candidates and allocations", () => {
  ...
  expect(mocks.loadMore).toHaveBeenCalledTimes(8);
