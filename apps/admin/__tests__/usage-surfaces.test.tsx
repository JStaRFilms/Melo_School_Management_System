...
vi.mock("convex/react", () => ({ useQuery: (ref: Parameters<typeof getFunctionName>[0], args: unknown) => {
...
it("has denied/loading/no-entitlement states and never invents purchase or prices", () => {
  ...
  render(<Usage />);
  expect(screen.getByText(/No allowance recorded/)).toBeTruthy();
  ...
  expect(screen.getByRole("button", { name: /Request exception/ }).hasAttribute("disabled")).toBe(true);
});
