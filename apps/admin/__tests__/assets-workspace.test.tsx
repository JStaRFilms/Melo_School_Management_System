...
const state = vi.hoisted(() => ({ allowed: true as boolean | undefined, archiveAllowed: true, area: "library", error: false, empty: false, held: false, mutate: vi.fn() }));
...
afterEach(() => { cleanup(); state.allowed = true; state.archiveAllowed = true; state.area = "library"; state.error = false; state.empty = false; state.held = false; state.mutate.mockReset(); });
...
it("hides Archive and denies the route before mounting its privileged query", () => {
  state.archiveAllowed = false;
  render(<AssetsWorkspace area="library" />);
  expect(screen.queryByRole("link", { name: "Asset Archive" })).toBeNull();
  cleanup();
  render(<AssetsWorkspace area="archive" />);
  expect(screen.getByRole("alert").textContent).toContain("Archive access denied");
