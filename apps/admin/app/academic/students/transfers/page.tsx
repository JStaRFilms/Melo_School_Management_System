...
function Review({
  ...
  const reverseCompleted = useMutation(transfers.reverseCompletedTransfer);
  const [classId, setClassId] = useState<Id<"classes">>();
  ...
  return (
    <section className="space-y-3 rounded border p-4">
                ...
                onClick={() => {
                  ...
                  const args = {
                    ...
                    expectedCounterVersion: number?.available
                      ...
                      : undefined,
                    expectedResetPeriod: number?.available
                      ? number.resetPeriod
                      : undefined,
      ...
      {record.status === "completed" && (
        <fieldset disabled={pending || !!retry} className="space-y-3 rounded border border-amber-300 p-3">
          <legend className="font-medium">Completed-transfer correction</legend>
          <p>This governed reversal requires transfer authority in both branches. It restores the source enrollment, archives the destination projection, and permanently preserves the destination admission-number history.</p>
          <label>
            Correction reason
            <input className={control} minLength={12} maxLength={500} value={reason} onChange={(event) => { setReason(event.target.value); setConfirmed(false); }} />
          </label>
          <label className="block">
            <input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />{" "}
            I reviewed both branch records and confirm this completed-transfer reversal.
          </label>
          <button
            className="rounded border border-amber-500 p-2"
            disabled={!confirmed || reason.trim().length < 12}
            onClick={() => void perform(() => reverseCompleted({ transferId, reason, confirmation: "REVERSE COMPLETED TRANSFER" }))}
          >
