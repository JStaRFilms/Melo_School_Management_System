...
export default function CurriculumReadinessPage() {
  ...
  const readiness = useQuery(
    ...
    subjectId && level && activeTerm ? { subjectId, level: level.trim(), termId: activeTerm._id } as never : "skip" as never,
  ) as CurriculumReadinessResponse | undefined;
  const isLoading = Boolean(subjectId && level && activeTerm) && readiness === undefined;
  const contextReady = Boolean(subjectId && level && activeTerm);
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-5 p-4 sm:p-6 lg:p-8">
      <AdminHeader
        ...
        title="Readiness map"
        description="See preparation evidence for approved curriculum topics in the current academic context."
      />
      <ReadinessContextBar
      ...
      />
      {!activeSession || !activeTerm ? <ContextNotice message="Set an active session and term before reviewing curriculum readiness." /> : null}
      {activeTerm && (!subjectOptions.length || !levelOptions.length) ? <ContextNotice message="Add at least one subject and class before viewing a readiness map." /> : null}
