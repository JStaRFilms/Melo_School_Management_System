...
interface Props { unit: CurriculumUnit; busy: boolean; onReview: (unit: CurriculumUnit, action: "proposed" | "rejected") => void; onApprove: (unit: CurriculumUnit) => void; }
export function CurriculumUnitCard({ unit, busy, onReview, onApprove }: Props) {
  const warnings = [...unit.validationWarnings, ...unit.duplicateWarnings];
  return <article className="border-b border-slate-100 py-4 last:border-0">
    ...
    {unit.reviewStatus !== "approved" && <div className="mt-3 flex flex-wrap gap-2"><button disabled={busy} onClick={() => onReview(unit, "proposed")} className="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-wider text-slate-600"><Pencil className="h-3 w-3" /> Edit</button><button disabled={busy} onClick={() => onReview(unit, "rejected")} className="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-wider text-rose-600"><X className="h-3 w-3" /> Reject</button><button disabled={busy} onClick={() => onApprove(unit)} className="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-2 py-1.5 text-[9px] font-black uppercase tracking-wider text-white"><Check className="h-3 w-3" /> Approve</button></div>}
  </article>;
}
