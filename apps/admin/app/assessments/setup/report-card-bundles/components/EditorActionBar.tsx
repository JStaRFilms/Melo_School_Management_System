...
export function EditorActionBar({
  ...
  return (
    <>
      {result && (
        <div className="fixed right-6 bottom-24 z-[100] animate-in slide-in-from-right-4 duration-500">
          <div className={`flex items-center gap-3 rounded-2xl px-6 py-4 shadow-2xl backdrop-blur-xl border ${
            result.success 
            ...
            <button className="ml-2 p-1 hover:bg-white/10 rounded-lg" onClick={() => setResult(null)}>
              <X className="h-3 w-3" />
            </button>
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 p-4 lg:p-6 bg-gradient-to-t from-slate-50 via-slate-50/80 to-transparent pointer-events-none">
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            ...
            >
              {isSaving ? (
                ...
                <UploadCloud className={`w-4 h-4 ${dirty ? "text-emerald-400" : "opacity-20"}`} />
              )}
