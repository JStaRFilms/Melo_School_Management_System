...
import { getUserFacingErrorMessage, appToast } from "@school/shared";
import {
  Hash,
  Save,
  AlertCircle,
...
export default function AdmissionNumberingPage() {
  ...
  const run = async (operation: () => Promise<unknown>, success: string) => {
    ...
    try {
      await operation();
      appToast.success("Numbering policy saved", {
        description: success,
      });
      setFeedback({
        type: "success",
      ...
      appToast.error(formatted.title, {
        description: formatted.message,
        action:
          formatted.action === "resetSequence"
            ? {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6 pb-24">
      ...
      {/* Status announcement for screen readers & tests */}
      {feedback && (
