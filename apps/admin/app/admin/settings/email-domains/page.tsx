...
function EmailAccess({ schoolId, accountId }: { schoolId: Id<"schools">; accountId?: string }) {
  ...
  if (!policy && !staff && !student && !lifecycle) return <p role="alert">Email settings access denied. Ask the proprietor for scoped registrar, staff administrator, lifecycle, or domain policy authority.</p>;
...
function EmailWorkbench({ schoolId, accountId }: { schoolId: Id<"schools">; accountId?: string }) {
  ...
  const sharedDomainsBefore = usePaginatedQuery(emailApi.listSharedEmailDomainsPage, data?.groupName ? { schoolId, ownerRange: "before" } : "skip", { initialNumItems: 25 });
  const sharedDomainsAfter = usePaginatedQuery(emailApi.listSharedEmailDomainsPage, data?.groupName ? { schoolId, ownerRange: "after" } : "skip", { initialNumItems: 25 });
  const proposalPeople = usePaginatedQuery(emailApi.listEmailProposalPeoplePage, { schoolId }, { initialNumItems: 25 });
  const staffMailboxes = usePaginatedQuery(
    emailApi.listInstitutionalMailboxesPage,
    data?.permissions.staff || data?.permissions.lifecycle ? { schoolId, recipientKind: "staff" } : "skip",
    { initialNumItems: 25 },
  );
  const studentMailboxes = usePaginatedQuery(
    emailApi.listInstitutionalMailboxesPage,
    data?.permissions.student ? { schoolId, recipientKind: "student" } : "skip",
    { initialNumItems: 25 },
  );
  const legacyMailboxes = usePaginatedQuery(
    emailApi.listLegacyInstitutionalMailboxesPage,
    data ? { schoolId } : "skip",
    { initialNumItems: 25 },
  );
  const policyDomain = data?.policyDomain;
  const domains = useMemo(() => [...new Map([
    ...(policyDomain ? [policyDomain] : []),
    ...ownedDomains.results,
    ...sharedDomainsBefore.results,
    ...sharedDomainsAfter.results,
  ].map(domain => [domain._id, domain])).values()], [policyDomain, ownedDomains.results, sharedDomainsBefore.results, sharedDomainsAfter.results]);
  const people = proposalPeople.results;
  const mailboxes = [...staffMailboxes.results, ...studentMailboxes.results, ...legacyMailboxes.results];
  useEffect(() => {
    if (proposalPeople.status === "CanLoadMore" && people.length < 25)
      proposalPeople.loadMore(25);
  }, [proposalPeople.status, proposalPeople.loadMore, people.length]);
  ...
  const defaultDomain = domains.find(d => d._id === data.policy?.domainId)
    ?? domains.find(d => d.schoolId === schoolId && d.isDefault)
    ?? domains[0];
  ...
  return <main className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6 pb-20 text-slate-900">
    ...
    <section className="space-y-3" aria-labelledby="domains-title"><h2 id="domains-title" className="font-semibold">Domains and shared-group inheritance</h2>
      {!domains.length && ownedDomains.status === "Exhausted" && sharedDomainsBefore.status === "Exhausted" && sharedDomainsAfter.status === "Exhausted" && <p>No domains registered. A domain policy administrator must register one before address review.</p>}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {(sharedDomainsBefore.status === "CanLoadMore" || sharedDomainsAfter.status === "CanLoadMore") && <button className={button} onClick={() => { if (sharedDomainsBefore.status === "CanLoadMore") sharedDomainsBefore.loadMore(25); if (sharedDomainsAfter.status === "CanLoadMore") sharedDomainsAfter.loadMore(25); }}>Load more shared domains</button>}
      </div>
    ...
    <section className="space-y-3" aria-labelledby="lifecycle-title"><h2 id="lifecycle-title" className="font-semibold">Allocations, lifecycle and reconciliation</h2>
      ...
      {!mailboxes.length && staffMailboxes.status === "Exhausted" && studentMailboxes.status === "Exhausted" && legacyMailboxes.status === "Exhausted" && <p>No approved address allocations.</p>}
      ...
      {(staffMailboxes.status === "CanLoadMore" || studentMailboxes.status === "CanLoadMore" || legacyMailboxes.status === "CanLoadMore") && <button className={button} onClick={() => { if (staffMailboxes.status === "CanLoadMore") staffMailboxes.loadMore(25); if (studentMailboxes.status === "CanLoadMore") studentMailboxes.loadMore(25); if (legacyMailboxes.status === "CanLoadMore") legacyMailboxes.loadMore(25); }}>Load more allocations</button>}
    </section>
  </main>;
}
