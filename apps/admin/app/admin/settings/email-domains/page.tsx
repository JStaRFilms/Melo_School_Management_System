...
function EmailWorkbench({ schoolId }: { schoolId: Id<"schools"> }) {
  ...
  const staffMailboxes = usePaginatedQuery(emailApi.listInstitutionalMailboxesPage, data?.permissions.staff || data?.permissions.lifecycle ? { schoolId, recipientKind: "staff" } : "skip", { initialNumItems: 100 });
  const studentMailboxes = usePaginatedQuery(emailApi.listInstitutionalMailboxesPage, data?.permissions.student ? { schoolId, recipientKind: "student" } : "skip", { initialNumItems: 100 });
  const legacyMailboxes = usePaginatedQuery(emailApi.listLegacyInstitutionalMailboxesPage, { schoolId }, { initialNumItems: 100 });
  ...
  return <main className="mx-auto max-w-4xl space-y-6 p-4 text-slate-900">
    ...
    <section className="space-y-3" aria-labelledby="lifecycle-title"><h2 id="lifecycle-title" className="font-semibold">Allocations, lifecycle and reconciliation</h2>
      ...
      {!mailboxes.length && <p>No approved address allocations loaded.</p>}
      <div className="flex flex-wrap gap-2">{staffMailboxes.status === "CanLoadMore" && <button className={button} onClick={() => staffMailboxes.loadMore(100)}>Load more staff mailboxes</button>}{studentMailboxes.status === "CanLoadMore" && <button className={button} onClick={() => studentMailboxes.loadMore(100)}>Load more student mailboxes</button>}{legacyMailboxes.status === "CanLoadMore" && <button className={button} onClick={() => legacyMailboxes.loadMore(100)}>Load more legacy mailboxes</button>}</div>
