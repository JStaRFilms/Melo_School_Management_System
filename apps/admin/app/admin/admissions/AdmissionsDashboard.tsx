...
const fieldClass = "w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-950 focus:border-sky-600 focus:outline-none focus:ring-2 focus:ring-sky-200";
const buttonClass = "rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50";
...
export function AdmissionsDashboard() {
  ...
  return <main className="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6">
    <AdminHeader label="Enrollment" title="Admissions" description="Manage published application campaigns and review the redacted application queue." actions={<Link className={buttonClass} href="/admin/admissions/retention">Retention</Link>} />
    <p role="status" aria-live="polite" className="rounded-lg bg-slate-100 px-3 py-2 text-sm text-slate-700">{feedback}</p>
    {canManage ? <AdminSurface as="section" className="space-y-4 p-4 sm:p-5">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-display text-lg font-bold">Campaigns and forms</h2><p className="text-sm text-slate-600">Draft changes never alter the version already bound to an application.</p></div><button className={buttonClass} onClick={() => begin()}>New campaign</button></div>
      <div className="grid gap-3 lg:grid-cols-2">{campaigns?.map((item) => <article key={`${item.formVersionId}:${item.lifecycle}`} className="rounded-lg border border-slate-200 p-3"><div className="flex justify-between gap-2"><div><h3 className="font-semibold">{item.programmeName} — {item.intakeName}</h3><p className="text-sm text-slate-600">Form v{item.formVersion} · {item.lifecycle} · {item.currency} {(item.amountMinor / 100).toLocaleString()}</p></div><span className="text-xs font-bold uppercase text-slate-500">{item.lifecycle}</span></div><div className="mt-3 flex flex-wrap gap-2">{item.lifecycle === "draft" ? <><button className={buttonClass} onClick={() => begin(item)}>Edit</button><button className={buttonClass} onClick={() => setConfirm({ action: "publish", campaign: item })}>Publish</button></> : <><button className={buttonClass} onClick={() => begin(item, true)}>New version</button><button className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold" onClick={() => void navigator.clipboard.writeText(item.applicationLink.href).then(() => setFeedback("Apply link copied."), () => setFeedback("The Apply link could not be copied. Open it and copy from the address bar."))}>Copy Apply link</button><a className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold" href={item.applicationLink.href} target="_blank" rel="noreferrer">Open Apply link</a><button className="rounded-lg border border-rose-300 px-4 py-2 text-sm font-semibold text-rose-700" onClick={() => setConfirm({ action: "close", campaign: item })}>Close</button></>}</div></article>)}{campaigns?.length === 0 ? <p className="text-sm text-slate-600">No campaigns yet.</p> : null}</div>
    ...
    {canList ? <AdminSurface as="section" className="space-y-4 p-4 sm:p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="font-display text-lg font-bold">Application queue</h2><p className="text-sm text-slate-600">This list is redacted. Open a record for submitted snapshot detail.</p></div><label className="text-sm font-semibold">State<select className={`${fieldClass} mt-1`} value={queueState} onChange={(event) => { setQueueState(event.target.value as typeof queueState); setCursor(null); }}>{["submitted", "under_review", "changes_requested", "waitlisted", "accepted", "rejected"].map((state) => <option key={state}>{state}</option>)}</select></label></div><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead><tr className="border-b"><th className="p-2">Reference</th><th className="p-2">State</th><th className="p-2">Revision</th><th className="p-2">Updated</th></tr></thead><tbody>{queue?.page.map((item) => <tr key={item.applicationId} className="border-b"><td className="p-2"><Link className="font-semibold text-sky-700 underline" href={`/admin/admissions/${encodeURIComponent(item.publicId)}`}>{item.publicId}</Link></td><td className="p-2">{item.state}</td><td className="p-2">{item.currentRevision}</td><td className="p-2">{new Date(item.updatedAt).toLocaleString()}</td></tr>)}</tbody></table></div>{queue && !queue.isDone ? <button className={buttonClass} onClick={() => setCursor(queue.continueCursor)}>Next page</button> : null}</AdminSurface> : null}
...
function CampaignForm({ values, update, errors, busy, replacement, conflict, onReload, onSave, onCancel }: { values: CampaignEditorValues; update: (key: keyof CampaignEditorValues, value: string) => void; errors: string[]; busy: boolean; replacement: boolean; conflict: boolean; onReload: () => void; onSave: () => void; onCancel: () => void }) {
  const fields: Array<[keyof CampaignEditorValues, string, string]> = [["programmeSlug", "Programme slug", "text"], ["programmeName", "Programme name", "text"], ["intakeSlug", "Intake slug", "text"], ["intakeName", "Intake name", "text"], ["cycleLabel", "Cycle label", "text"], ["opensAt", "Opens", "datetime-local"], ["closesAt", "Closes", "datetime-local"], ["productSlug", "Product slug", "text"], ["productName", "Fee product name", "text"], ["amount", "Fee amount", "number"], ["currency", "Currency", "text"], ["refundPolicyKey", "Refund policy key", "text"]];
  const replacementImmutable = new Set<keyof CampaignEditorValues>(["programmeSlug", "programmeName", "intakeSlug", "intakeName", "cycleLabel", "opensAt", "closesAt", "productSlug", "productName"]);
  return <form className="space-y-4 rounded-xl border border-slate-300 bg-slate-50 p-4" onSubmit={(event) => { event.preventDefault(); onSave(); }}><h3 className="font-bold">{replacement ? "Replacement version" : "Campaign draft"}</h3>{replacement ? <p className="text-sm text-slate-600">Campaign identity, intake dates, and product identity are immutable in a replacement version. Create a new campaign to change them.</p> : null}<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{fields.map(([key, label, type]) => <label key={key} className="text-sm font-semibold">{label}<input required disabled={replacement && replacementImmutable.has(key)} className={`${fieldClass} mt-1 disabled:bg-slate-200`} type={type} min={type === "number" ? "0.01" : undefined} step={type === "number" ? "0.01" : undefined} value={values[key]} onChange={(event) => update(key, event.target.value)} /></label>)}</div>{[["declarationTitle", "Declaration title"], ["declarationBody", "Declaration body"], ["declarationPurpose", "Declaration purpose"], ["feeDisclosure", "Fee disclosure"], ["fieldsJson", "Form fields (JSON array)"], ["requirementsJson", "Document requirements (JSON array)"]].map(([key, label]) => <label key={key} className="block text-sm font-semibold">{label}<textarea required className={`${fieldClass} mt-1 min-h-20 font-mono`} value={values[key as keyof CampaignEditorValues]} onChange={(event) => update(key as keyof CampaignEditorValues, event.target.value)} /></label>)}{errors.length ? <div role="alert" className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900"><p className="font-bold">Resolve these validation issues:</p><ul className="list-disc pl-5">{errors.map((error) => <li key={error}>{error}</li>)}</ul></div> : null}<div className="flex flex-wrap gap-2">{conflict ? <button type="button" className="rounded-lg border border-amber-400 px-4 py-2 text-sm font-semibold" onClick={onReload}>Reload all server values</button> : null}<button className={buttonClass} disabled={busy || conflict || errors.length > 0}>Save draft</button><button type="button" className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold" onClick={onCancel}>Cancel</button></div></form>;
}
