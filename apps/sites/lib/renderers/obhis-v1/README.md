...
Before release, connect an approved B0 public projection for an OBHIS managed profile and complete the approval ledger. The renderer now supplies a route-availability predicate so unknown policy slugs fail before metadata/sitemap generation. Public projections use `no-store` so a newly immutable revision and current application availability are visible on the next request. The public asset projection accepts optional approved width, height, focal-point, and optimized `responsiveSources` metadata; B6 should supply those values so the renderer can emit dimension-reserved responsive `srcset` delivery.
