...
Before release, connect an approved B0 public projection for an OBHIS managed profile and complete the approval ledger. The renderer now supplies a route-availability predicate so unknown policy slugs fail before metadata/sitemap generation. B6 must call `invalidatePublishedSiteCache(schoolId)` from the reviewed publish/revert projection workflow so a newly immutable revision is visible immediately, rather than waiting for the bounded TTL. The public asset projection now accepts optional approved width, height, and focal-point metadata; B6 should ensure its projection supplies those values for responsive-image performance.
