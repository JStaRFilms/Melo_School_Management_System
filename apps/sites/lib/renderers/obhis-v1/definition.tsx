...
function isObhisRouteAvailable(data: ObhisRendererData, routeKey: string, params: Readonly<Record<string, string>>, applicationAvailability: string) {
  const admissionsAvailable = Boolean(data.admissions.lead && data.admissions.steps.length && applicationAvailability === "open");
  const visitAvailable = Boolean(data.contact.address);
  switch (routeKey) {
...
export const obhisRenderer: SiteRenderer<ObhisRendererData> = {
  ...
  getPresentation(data, context) {
    ...
    return { ...(policy ? { title: `${policy.title} — ${data.identity.displayName}`, ...(policy.summary ? { description: policy.summary } : {}) } : {}), structuredData: { "@context": "https://schema.org", "@graph": [{ "@type": "WebSite", name: data.identity.displayName, url: root }, organization] } };
  },
...
function ObhisSite({ context }: { context: SiteRenderContext<ObhisRendererData> }) {
  ...
  return <div className={styles.site}>
    ...
    <ObhisNavigation name={data.identity.shortName ?? data.identity.displayName} homeHref={path("/")} items={nav} applicationHref={applicationHref} portalHref={context.links.portal?.href} />
