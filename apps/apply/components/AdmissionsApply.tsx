...
import { answerDisplay, answerPayload, availabilityMessage, conditionMatchesAnswers, correctionAllows, dateInputToUtcTimestamp, documentSelectionError, fieldOptions, formatFileSize, formatMoney, isDraftConflict, missingRequiredItemLabels, paymentMessage, validateSubmissionInput } from "@/lib/journey";
...
export function guardianFacingErrorMessage(error: unknown) {
  ...
  if (/FORBIDDEN|UNAUTHORIZED/i.test(raw)) return "You do not have access to complete this action.";
  if (/\[CONVEX|ConvexError|Called by client/i.test(raw)) return "We could not complete that request. Please try again.";
...
export function GuardianAccount({ schoolSlug }: { schoolSlug: string }) {
  ...
  const catalogue = useQuery(listPublishedOfferingsRef, { schoolSlug, now: useOfferingNow() });
  const createAttempt = useMutation(createAttemptRef), initialize = useAction(initializeAttemptRef), reserve = useMutation(createOrResumeApplicationRef);
  const [status, setStatus] = useState("Your payment and application states come from server records."), [busy, setBusy] = useState(false);
  ...
  async function checkout(productSlug: string) { setBusy(true); setStatus("Creating a purchase attempt…"); try { const attempt = await createAttempt({ schoolSlug, productSlug, idempotencyKey: key() }); setStatus(paymentMessage(attempt.state)); const initialized = await initialize({ reference: attempt.reference, returnOrigin: window.location.origin }); setStatus(paymentMessage(initialized.state)); window.location.assign(initialized.authorizationUrl); } catch (error) { const text = errorMessage(error); setStatus(text); appToast.error("Checkout could not start", { description: text }); setBusy(false); } }
  async function start(entitlementId: Id<"admissionsEntitlements">) { setBusy(true); setStatus("Starting this child’s application…"); try { const application = await reserve({ entitlementId }); router.push(`/s/${encodeURIComponent(schoolSlug)}/applications/${encodeURIComponent(application.publicId)}`); } catch (error) { setStatus(errorMessage(error)); setBusy(false); } }
  return <Frame schoolSlug={schoolSlug}><main><section className="apply-card"><h1>My applications</h1><p role="status" aria-live="polite" className="status">{guardian.error || status}</p>{guardian.error ? <VerificationRecovery email={guardian.session.user.email} retry={guardian.retry} /> : guardian.loading || workspace === undefined ? <p>Loading owned slots and applications…</p> : <div className="apply-grid two">{workspace.applications.map((application) => <article className="apply-card" key={application.applicationId}><span className={`pill ${application.state}`}>{statusLabel(application.state)}</span><h2>Application {application.publicId}</h2><p>Revision {application.currentRevision} · draft version {application.draftVersion}</p><Link className="primary" href={`/s/${encodeURIComponent(schoolSlug)}/applications/${encodeURIComponent(application.publicId)}`}>Resume or view status</Link></article>)}{workspace.entitlements.filter((slot) => slot.state === "available").map((slot) => <article className="apply-card" key={slot.entitlementId}><span className="pill paid">paid slot</span><h2>One-child application slot</h2><button className="primary" disabled={busy} onClick={() => void start(slot.entitlementId)}>Start this child’s application</button></article>)}</div>}</section><section className="apply-card"><h2>Purchase attempts</h2>{workspace?.attempts.length ? workspace.attempts.map((attempt) => <article className="offer" key={attempt.reference}><strong>{formatMoney(attempt.amountMinor, attempt.currency)}</strong><span className={`pill ${attempt.state}`}>{statusLabel(attempt.state)}</span><p>{paymentMessage(attempt.state)}</p></article>) : <p className="muted">No purchase attempts yet.</p>}</section><section className="apply-card"><h2>Buy another one-child slot</h2>{catalogue?.available ? catalogue.offerings.map((item) => { if (item.availability !== "open" || !item.productSlug || item.amountMinor === null || !item.currency) return null; const productSlug = item.productSlug; return <article className="offer" key={item.intakeSlug}><strong>{item.productName} · {formatMoney(item.amountMinor, item.currency)}</strong><p>{item.feeDisclosure}</p><button className="primary" disabled={busy} onClick={() => void checkout(productSlug)}>Pay with Paystack</button></article>; }) : <p className="muted">No purchasable offering is currently available.</p>}</section></main></Frame>;
}

export function PaystackReturn({ schoolSlug, reference }: { schoolSlug: string; reference: string | null }) {
  const guardian = useGuardianReady(); const verify = useAction(verifyReturnRef); const [state, setState] = useState<"idle" | "checking" | "done" | "error">("idle"), [status, setStatus] = useState("Waiting to verify the owned payment on the server."), [checks, setChecks] = useState(0);
  useEffect(() => { if (!guardian.ready || !reference || state !== "idle" || checks >= 3) return; setState("checking"); setChecks((value) => value + 1); setStatus("Verifying payment with Paystack on the server…"); void verify({ reference }).then((result) => { if (result.state === "checkout_pending" || result.state === "created") { setState("error"); setStatus("Payment is still pending provider verification. You may check again."); } else { setState("done"); setStatus(paymentMessage(result.state)); } }).catch((error) => { setState("error"); setStatus(errorMessage(error)); }); }, [checks, guardian.ready, reference, state, verify]);
  return <Frame schoolSlug={schoolSlug}><section className="apply-card"><h1>Payment return</h1>{!reference ? <p className="status error">No payment reference was provided. No payment claim has been accepted.</p> : <p className={`status ${state === "done" ? "success" : state === "error" ? "error" : ""}`} role="status" aria-live="polite">{status}</p>}<p className="muted">This page never trusts success text in the URL. A paid state appears only after backend verification. Verification checks are limited to three per page visit.</p>{state === "error" && checks < 3 ? <button className="secondary" onClick={() => setState("idle")}>Check payment again</button> : null}{state === "error" && checks >= 3 ? <p className="status warn">The check limit was reached. Return to your applications and try again later.</p> : null}<Link className="primary" href={`/s/${encodeURIComponent(schoolSlug)}/account`}>Return to my applications</Link></section></Frame>;
}
...
function ApplicationForm({ schoolSlug, applicationId, draft, financialHold, safeMessages, conversion }: { schoolSlug: string; applicationId: Id<"admissionsApplications">; draft: Draft; financialHold: boolean; safeMessages: string[]; conversion: { state: "processing" | "completed" | "needs_attention"; message: string; admissionNumber: string | null } | null }) {
  ...
  function adoptServerDraft() { setProfile({ firstName: draft.profile?.firstName ?? "", middleName: draft.profile?.middleName ?? "", lastName: draft.profile?.lastName ?? "", dateOfBirth: draft.profile ? new Date(draft.profile.dateOfBirth).toISOString().slice(0, 10) : "" }); setContact({ fullName: draft.primaryContact?.fullName ?? "", relationship: draft.primaryContact?.relationship ?? "", email: draft.primaryContact?.email ?? "", phone: draft.primaryContact?.phone ?? "" }); setEntry(draft.requestedEntryLabel ?? ""); setAnswers(Object.fromEntries(draft.answers.map((answer) => [answer.fieldKey, answerDisplay(draft.form.fields.find((field) => field.fieldKey === answer.fieldKey)?.kind ?? "text", answer.serializedValue)]))); setVersion(draft.draftVersion); setConflict(false); }
  useEffect(() => { if (initialized.current) return; initialized.current = true; adoptServerDraft(); }, [draft]);
  ...
  async function uploadFile(requirementId: Id<"admissionsDocumentRequirements">, file: File) {
    ...
    const replacementAllowed = draft.state === "changes_requested" && requirement.maxFiles === 1 && activeFileCount === 1 && Boolean(draft.correction?.requirementIds.includes(requirementId));
...
export function PublishedField({ field, value, disabled, onChange, onClear }: { field: Draft["form"]["fields"][number]; value: string; disabled: boolean; onChange: (value: string) => void; onClear?: () => void }) {
  ...
  else if (field.kind === "multi_select") { const selected = value.split(",").map((item) => item.trim()).filter(Boolean); control = <select multiple disabled={disabled} required={required} value={selected} onChange={(event) => onChange(Array.from(event.currentTarget.selectedOptions, (option) => option.value).join(", "))}>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select>; }
