...
export function ApplicationSurface({ schoolSlug, publicReference }: { schoolSlug: string; publicReference: string }) {
  ...
  const sections = [...((config?.sections ?? []) as Section[])].sort((left, right) => left.order - right.order);
  const formSteps = ["child", "contacts", ...sections.map((section) => section.key), "documents", "review"];
  const fields = (config?.fields ?? []).filter((field: Field) => field.sectionKey === step && fieldIsVisible(field, answers));
  const sectionLabel = sections.find((section) => section.key === step)?.label;
  ...
  const contactEditable = editable && (!changesRequested || app.permittedEdits.coreKeys.some((key: string) => key.startsWith("contact:")));
  const stepHasCorrection = (section: string) => correctionStepHasEditableItems({
    state: app.state,
    section,
    coreKeys: app.permittedEdits.coreKeys,
    fieldKeys: app.permittedEdits.fieldKeys,
    requirementKeys: app.permittedEdits.requirementKeys,
  ...
  const stepperElement = (
    <ol className="stepper">
      {formSteps.map(item => {
        ...
        const label = item === "child" ? "Child and form" : item === "contacts" ? "Guardian contact" : item === "documents" ? "Documents" : item === "review" ? "Review and declaration" : sections.find((section) => section.key === item)?.label ?? item.replace(/[-_]/g, " ");
        return (
          <li key={item}>
            <button
              type="button"
