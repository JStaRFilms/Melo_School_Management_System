...
export const handler = authServer?.handler ?? { GET: async () => new Response("Authentication is not configured.", { status: 503 }), POST: async () => new Response("Authentication is not configured.", { status: 503 }) };
