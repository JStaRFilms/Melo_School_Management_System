...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Persistence is optional; both server contracts remain authoritative.
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Keep this session fail-closed even when stale persistence cannot be cleaned up.
    }
  ...
  const clearSelectedSchool = useCallback(() => {
    if (accountId) {
      try {
        ...
        // In-memory reset still returns to the server-resolved default.
      }
    }
    setSelectedSchoolId(null);
  }, [accountId]);
