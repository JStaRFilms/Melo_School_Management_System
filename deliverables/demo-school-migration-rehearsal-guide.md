...
- Read demo credentials only from the operator-provisioned, intentionally untracked local file `tmp/demo_school_credentials.md` after authorization. If it is absent, stop and ask the credential custodian; do not create or commit a replacement. Never echo, paste, screenshot, or place credentials in this HTML checklist or browser storage.
