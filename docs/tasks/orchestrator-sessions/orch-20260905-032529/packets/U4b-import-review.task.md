...
## Execution note
**IMPLEMENTED — R1 Critical 1 locally remediated; runtime evidence remains E0.** The routed workbench now uses creator-private, revisioned row decisions (`create_new` / `merge_existing` / `ignore`), explicit existing-entity mappings, immutable reviewed plans, H4 missing-only proposals/allocation, reviewed historical-number counter choices, deterministic commit-time revalidation, bounded audited receipts, and idempotent reconciliation. Identity/entity fabrication and the legacy importer commit are hard-disabled; source-file upload and provider AI remain truthfully unavailable rather than simulated. See `../results/R1-import-remediation.md` for the code contract, regression matrix, checks, and remaining runtime gates.
