...
**Still gated/not claimed by this authorization patch:** capability-only shells without reviewed legacy projections, selected-branch activation, unmapped legacy-domain adoption, runtime/browser/accessibility/print evidence, schema rollout and reviewed cutover/backfill. Later stack changes close the reviewed import-commit correctness gap with immutable reviewed decisions, authoritative numbering, audit/reconciliation receipts, and bounded retry behavior. Upload transport remains deliberately fail-closed pending entitlement reservation, trusted provenance, cleanup, and provider activation; raw legacy uploads are not a unified quota/provider-ownership transport merely because their permission and asset-boundary checks improved.
