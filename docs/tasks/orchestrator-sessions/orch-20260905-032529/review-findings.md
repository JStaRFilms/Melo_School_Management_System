...
> **Current reconciliation at this S0 boundary:** Critical 2 and 3 were remediated by the authoritative capability and Platform-isolation work recorded in `results/R1-security.md` and the later lifecycle remediation results. Critical 1 remains an active blocker here because the public import commit path can still reuse an admission-number collision; its correction belongs to the later integration PR, not this stabilization boundary. Critical 4 is locally contained by server-side fail-closed upload issuance/finalization for assets, logos, student photos, staff/Portal knowledge uploads, and PDF candidates. The generic Convex URL is not treated as provenance or quota evidence. Existing authorized historical reads remain compatibility-only. See `results/S0-storage.md`. Upload functionality remains unavailable pending authoritative transport/provider activation, so that feature is explicitly incomplete rather than an unresolved authorization bypass.

The findings below preserve the point-in-time evidence that triggered remediation. At this boundary, Critical 1 remains open and Critical 4 remains externally gated as described above. Remaining limitations and external gates are tracked in the later result documents.
