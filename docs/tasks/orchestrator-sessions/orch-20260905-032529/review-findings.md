...
> **Current reconciliation:** Critical 2 and 3 were remediated by the authoritative capability and Platform-isolation work recorded in `results/R1-security.md` and the later lifecycle remediation results. Later stack changes also close Critical 1 with immutable reviewed import decisions, authoritative admission-number claims, audit/reconciliation receipts, and bounded retry behavior. Critical 4 is locally contained by server-side fail-closed upload issuance/finalization for assets, logos, student photos, staff/Portal knowledge uploads, and PDF candidates. The generic Convex URL is not treated as provenance or quota evidence. Existing authorized historical reads remain compatibility-only. See `results/S0-storage.md`. Upload functionality remains unavailable pending authoritative transport/provider activation, so that feature is explicitly incomplete rather than an unresolved authorization bypass.

The findings below preserve the point-in-time evidence that triggered remediation; they are not the current open-issue list. Remaining limitations and external gates are tracked in the later result documents.
...
8. **U4b (locally remediated):** reviewed existing-entity mappings, deterministic duplicate/relationship decisions, H4 numbering, audit receipts, reconciliation, retry, privacy-safe outcomes, and disabled unsafe legacy/file/provider paths are implemented. Runtime/browser evidence remains outstanding (Critical 1).
