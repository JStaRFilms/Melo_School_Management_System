...
- Blueprint includes API, UX behavior, file targets, non-goals, and acceptance
  criteria
...
- Use top-center as the proposed default unless audit reveals a strong reason
  not to
- Preserve inline validation where it gives field/table-level correction
  guidance
- Keep toast UI separate from product/domain notification center

## Notes

Task 02 completed. Created docs/features/unified-toast-system.md with shared
package audit, root layout targets, Sonner dependency approach, two-layer
system/UI architecture, API/import path, UX defaults, privacy-safe error
policy, migration rules, implementation order, non-goals, and acceptance
criteria. Noted that apps/platform must add @school/shared before importing
AppToaster.
