...
- `settings.getCatalogue`, `settings.createProgramme`, `settings.createIntake`, `settings.createProduct`, `settings.publishPrice`, `settings.createDraftForm`, `settings.addDraftField`, `settings.addDraftDocumentRequirement`, `settings.publishForm`, `settings.retireForm`, `settings.setProgrammeStatus`, `settings.setIntakeStatus`, `settings.setProductStatus`, and `settings.createDeclaration` are tenant-scoped persisted B3 catalogue/publication APIs. Status changes are explicit audited publication/rollback controls and never rewrite bound applications; published forms/declarations are immutable; sensitive configuration requires its explicit capability.
